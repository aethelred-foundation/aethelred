...
func writeSecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-counterparty-incident-directive-extension-appeal-reconciliation-challenge-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name", "snapshot_id",
			"bundle_id", "bundle_version", "bundle_name", "status", "verified", "signer", "key_id", "incident_id", "response_id", "directive_id",
			"extension_id", "dispute_id", "appeal_id", "comparison_key", "challenge_id", "challenge_appeal_id", "parent_challenge_appeal_id",
			"challenge_appeal_generation", "challenge_appeal_status", "appealing_party", "board_party", "enforcement_acknowledgement_party", "ruling",
			"board_review_threshold", "board_delegation_count", "board_recusal_count", "board_committee_member_count", "board_recorded_vote_count",
			"board_outstanding_votes", "board_missing_quorum_count", "board_threshold_satisfied", "ratify_vote_count", "overturn_vote_count",
			"generated_at", "expires_at", "received_at", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed",
			"portable_package_anchored", "verification_message", "matched_local_challenge_appeal_id", "alignment_status", "alignment_divergence_count",
			"review_status", "last_reviewed_by", "last_reviewed_at", "review_action_count",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				strconv.FormatBool(item.PortablePackageAnchored),
				item.VerificationMessage,
				item.MatchedLocalChallengeAppealID,
				string(item.AlignmentStatus),
				strconv.Itoa(item.AlignmentDivergenceCount),
				string(item.ReviewStatus),
				item.LastReviewedBy,
				secureCellCSVTime(item.LastReviewedAt),
				strconv.Itoa(item.ReviewActionCount),
...
func writeSecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-counterparty-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name", "snapshot_id",
			"bundle_id", "bundle_version", "bundle_name", "status", "verified", "signer", "key_id", "incident_id", "response_id", "directive_id",
			"extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id", "response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_appeal_status",
			"response_status", "response_action", "response_transition_id", "response_counterparty_reference", "response_counterparty_snapshot_id",
			"appealing_party", "correction_board_party", "enforcement_acknowledgement_party", "ruling", "board_review_threshold", "board_delegation_count", "board_recusal_count",
			"board_committee_member_count", "board_recorded_vote_count", "board_outstanding_votes", "board_missing_quorum_count", "board_threshold_satisfied",
			"ratify_vote_count", "overturn_vote_count", "generated_at", "expires_at", "received_at", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "verification_message", "matched_local_response_appeal_id",
			"alignment_status", "alignment_divergence_count", "review_status", "last_reviewed_by", "last_reviewed_at", "review_action_count",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.ParentResponseAppealID,
				strconv.Itoa(item.ResponseAppealGeneration),
				string(item.ResponseAppealStatus),
				string(item.ResponseStatus),
				string(item.ResponseAction),
				item.ResponseTransitionID,
				item.ResponseCounterpartyReference,
				item.ResponseCounterpartySnapshotID,
				string(item.AppealingParty),
				...
				strconv.FormatBool(item.PortablePackageAnchored),
				item.VerificationMessage,
				item.MatchedLocalResponseAppealID,
				string(item.AlignmentStatus),
				strconv.Itoa(item.AlignmentDivergenceCount),
				string(item.ReviewStatus),
				item.LastReviewedBy,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name", "incident_id",
			"response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id", "response_appeal_id",
			"local_response_appeal_status", "local_response_status", "local_response_action", "local_response_transition_id", "local_ruling",
			"counterparty_snapshot_id", "counterparty_bundle_id", "counterparty_response_appeal_id", "counterparty_response_appeal_status",
			"counterparty_response_status", "counterparty_response_action", "counterparty_response_transition_id", "counterparty_ruling",
			"counterparty_reference", "alignment_status", "alignment_divergence_count", "review_status", "action", "divergences", "transition_id",
			"policy_receipt_id", "policy_receipt_hash", "seal_id", "trace_link_id", "actor_did", "reason", "occurred_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.ChallengeAppealID,
				item.ResponseAppealID,
				string(item.LocalResponseAppealStatus),
				string(item.LocalResponseStatus),
				string(item.LocalResponseAction),
				item.LocalResponseTransitionID,
				string(item.LocalRuling),
				item.CounterpartySnapshotID,
				item.CounterpartyBundleID,
				item.CounterpartyResponseAppealID,
				string(item.CounterpartyResponseAppealStatus),
				string(item.CounterpartyResponseStatus),
				string(item.CounterpartyResponseAction),
				item.CounterpartyResponseTransitionID,
				string(item.CounterpartyRuling),
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-reviews.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"review_id", "cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "response_transition_id", "local_ruling",
			"counterparty_snapshot_id", "counterparty_bundle_id", "counterparty_bundle_status", "counterparty_bundle_verified", "counterparty_signer", "counterparty_response_appeal_id",
			"counterparty_response_appeal_status", "counterparty_response_status", "counterparty_response_action", "counterparty_response_transition_id", "counterparty_ruling", "counterparty_reference",
			"alignment_status", "alignment_divergence_count", "review_status", "latest_review_action", "last_reviewed_by", "last_reviewed_at", "review_action_count",
			"response_appeal_action_count", "response_appeal_recusal_count", "response_appeal_automation_action_count", "created_at", "received_at", "generated_at", "expires_at",
			"verification_message", "matched_local_response_appeal_id", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed", "portable_package_anchored",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.CounterpartyBundleID,
				string(item.CounterpartyBundleStatus),
				strconv.FormatBool(item.CounterpartyBundleVerified),
				item.CounterpartySigner,
				item.CounterpartyResponseAppealID,
				string(item.CounterpartyResponseAppealStatus),
				string(item.CounterpartyResponseStatus),
				string(item.CounterpartyResponseAction),
				...
				strconv.Itoa(item.AlignmentDivergenceCount),
				string(item.ReviewStatus),
				string(item.LatestReviewAction),
				item.LastReviewedBy,
				secureCellCSVTime(item.LastReviewedAt),
				strconv.Itoa(item.ReviewActionCount),
				strconv.Itoa(item.ResponseAppealActionCount),
				strconv.Itoa(item.ResponseAppealRecusalCount),
				strconv.Itoa(item.ResponseAppealAutomationCount),
				secureCellCSVTime(item.CreatedAt),
				item.ReceivedAt.UTC().Format(timeCSVFormat),
				item.GeneratedAt.UTC().Format(timeCSVFormat),
				secureCellCSVTime(item.ExpiresAt),
				item.VerificationMessage,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActionRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name",
			"snapshot_id", "bundle_id", "comparison_key", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id",
			"challenge_id", "challenge_appeal_id", "parent_challenge_appeal_id", "challenge_appeal_generation", "status", "alignment_status", "review_status", "action",
			"matched_local_challenge_appeal_id", "challenge_appeal_status", "appealing_party", "board_party", "enforcement_acknowledgement_party", "ruling",
			"board_review_threshold", "board_delegation_count", "board_recusal_count", "board_committee_member_count", "board_recorded_vote_count", "board_outstanding_votes",
			"board_missing_quorum_count", "board_threshold_satisfied", "ratify_vote_count", "overturn_vote_count", "verification_message", "divergences",
			"actor_did", "reason", "policy_receipt_id", "policy_receipt_hash", "seal_id", "trace_link_id", "transition_id", "occurred_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignments-overdue.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "organization_id", "snapshot_id", "bundle_id", "comparison_key", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id",
			"challenge_id", "challenge_appeal_id", "challenge_appeal_status", "status", "alignment_status", "review_status", "pending_action", "automation_action", "overdue_reason",
			"tier_id", "target_did", "matched_local_challenge_appeal_id", "verification_message", "alignment_divergence_count", "review_action_count", "due_at", "overdue_seconds", "received_at", "last_reviewed_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomationActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomationActionRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-automation-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "organization_id", "snapshot_id", "bundle_id", "comparison_key", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id",
			"challenge_id", "challenge_appeal_id", "challenge_appeal_status", "status", "alignment_status", "review_status", "pending_action", "matched_local_challenge_appeal_id",
			"verification_message", "alignment_divergence_count", "review_action_count", "contract_id", "contract_status_before", "contract_status_after", "action", "trigger", "tier_id", "target_did",
			"due_at", "actor", "automated_actor", "reason", "transition_id", "occurred_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "organization_id", "snapshot_id", "bundle_id", "comparison_key", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id",
			"challenge_id", "challenge_appeal_id", "challenge_appeal_status", "status", "alignment_status", "review_status", "response_status", "action", "matched_local_challenge_appeal_id",
			"verification_message", "divergences", "alignment_automation_action", "alignment_automation_trigger", "alignment_automation_pending_action", "alignment_automation_transition_id",
			"alignment_automation_actor", "alignment_automation_reason", "counterparty_reference", "counterparty_snapshot_id", "actor_did", "reason", "policy_receipt_id", "seal_id", "trace_link_id",
		...
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.VerificationMessage,
				strings.Join(item.Divergences, "|"),
				item.AlignmentAutomationAction,
				item.AlignmentAutomationTrigger,
				item.AlignmentAutomationPendingAction,
				item.AlignmentAutomationTransitionID,
				item.AlignmentAutomationActor,
				item.AlignmentAutomationReason,
				item.CounterpartyReference,
				item.CounterpartySnapshotID,
				item.ActorDID,
				item.Reason,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealSummary) error {
	...
	case "csv":
		...
		if err := cw.Write([]string{
			...
			"response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_status", "response_action", "response_transition_id", "status", "appealing_party", "correction_board_party", "enforcement_acknowledgement_party",
			"board_review_threshold", "board_committee_member_count", "board_delegation_count", "board_recusal_count", "board_recorded_vote_count", "board_outstanding_votes", "board_missing_quorum_count",
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionRecord) error {
	...
	case "csv":
		...
		if err := cw.Write([]string{
			"cell_id", "organization_id", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_status", "response_action", "status", "action", "correction_board_party", "delegated_to_did", "recusal_id",
			"board_review_threshold", "board_committee_member_count", "board_delegation_count", "board_recusal_count", "board_recorded_vote_count", "ratify_vote_count", "overturn_vote_count",
...
func writeSecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeals-overdue.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "organization_id", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_status", "response_action", "response_transition_id", "status", "appealing_party", "correction_board_party", "enforcement_acknowledgement_party",
			"pending_action", "automation_action", "overdue_reason", "board_review_threshold", "board_committee_member_count", "board_delegation_count", "board_recusal_count", "board_recorded_vote_count",
			"board_outstanding_votes", "board_missing_quorum_count", "board_threshold_satisfied", "ratify_vote_count", "overturn_vote_count", "ruling", "tier_id", "target_did", "due_at",
			"overdue_seconds", "created_at", "ruled_at", "updated_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActionRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-automation-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "organization_id", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			...
			"pending_action", "board_review_threshold", "board_committee_member_count", "board_delegation_count", "board_recorded_vote_count", "board_outstanding_votes",
			"board_missing_quorum_count", "board_threshold_satisfied", "ratify_vote_count", "overturn_vote_count", "ruling", "contract_id", "contract_status_before",
			"contract_status_after", "action", "trigger", "tier_id", "target_did", "due_at", "actor", "automated_actor", "reason", "transition_id", "occurred_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle) error {
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			...
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"challenge_appeal_status", "response_status", "response_action_count", "response_appeal_count", "response_appeal_status", "response_appeal_action_count", "response_appeal_automation_action_count", "counterparty_challenge_appeal_id", "counterparty_snapshot_id",
			...
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
			return err
		}
		signatureAlgorithm := ""
		...
		if bundle.CounterpartyChallengeAppeal != nil {
			counterpartyChallengeAppealID = bundle.CounterpartyChallengeAppeal.ChallengeAppealID
			counterpartySnapshotID = bundle.CounterpartyChallengeAppeal.SnapshotID
			counterpartySnapshotStatus = string(bundle.CounterpartyChallengeAppeal.Status)
		}
		if err := cw.Write([]string{
			bundle.ID,
			bundle.Version,
			...
			bundle.ChallengeAppealSummary.IncidentID,
			bundle.ChallengeAppealSummary.ResponseID,
			bundle.ChallengeAppealSummary.DirectiveID,
			bundle.ChallengeAppealSummary.ExtensionID,
			bundle.ChallengeAppealSummary.DisputeID,
			bundle.ChallengeAppealSummary.AppealID,
			bundle.ChallengeAppealSummary.ChallengeID,
			bundle.ChallengeAppealSummary.ChallengeAppealID,
			...
			strconv.Itoa(len(bundle.ResponseAppealAutomationActions)),
			counterpartyChallengeAppealID,
			counterpartySnapshotID,
			counterpartySnapshotStatus,
			strconv.Itoa(len(bundle.CounterpartyAlignmentActions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentAutomationActions)),
			strconv.Itoa(len(bundle.ResponseActions)),
			bundle.ChallengeAppealBundleHash,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response appeal counterparty review bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"review_id", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "local_ruling",
			"counterparty_snapshot_id", "counterparty_bundle_id", "counterparty_bundle_status", "counterparty_response_appeal_id", "counterparty_response_appeal_status",
			"counterparty_response_status", "counterparty_response_action", "counterparty_ruling", "alignment_status", "review_status", "latest_review_action",
			"review_action_count", "response_appeal_action_count", "response_appeal_recusal_count", "response_appeal_automation_action_count",
			"counterparty_action_count", "local_response_appeal_action_count", "local_response_appeal_recusal_count", "local_response_appeal_automation_action_count",
			"alignment_response_bundle_hash", "control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash",
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
			return err
		...
		if err := cw.Write([]string{
			...
			bundle.Organization.OrganizationName,
			string(bundle.Organization.Status),
			bundle.Review.ReviewID,
			bundle.Review.IncidentID,
			bundle.Review.ResponseID,
			bundle.Review.DirectiveID,
			bundle.Review.ExtensionID,
			bundle.Review.DisputeID,
			bundle.Review.AppealID,
			bundle.Review.ChallengeID,
			bundle.Review.ChallengeAppealID,
			bundle.Review.ResponseAppealID,
			string(bundle.Review.ResponseAppealStatus),
			string(bundle.Review.ResponseStatus),
			string(bundle.Review.ResponseAction),
			string(bundle.Review.LocalRuling),
			bundle.Review.CounterpartySnapshotID,
			bundle.Review.CounterpartyBundleID,
			string(bundle.Review.CounterpartyBundleStatus),
			bundle.Review.CounterpartyResponseAppealID,
			string(bundle.Review.CounterpartyResponseAppealStatus),
			string(bundle.Review.CounterpartyResponseStatus),
			string(bundle.Review.CounterpartyResponseAction),
			string(bundle.Review.CounterpartyRuling),
			string(bundle.Review.AlignmentStatus),
			string(bundle.Review.ReviewStatus),
			string(bundle.Review.LatestReviewAction),
			strconv.Itoa(bundle.Review.ReviewActionCount),
			strconv.Itoa(bundle.Review.ResponseAppealActionCount),
			strconv.Itoa(bundle.Review.ResponseAppealRecusalCount),
			strconv.Itoa(bundle.Review.ResponseAppealAutomationCount),
			strconv.Itoa(len(bundle.CounterpartyActions)),
			strconv.Itoa(len(bundle.LocalResponseAppealActions)),
			strconv.Itoa(len(bundle.LocalResponseAppealRecusals)),
			strconv.Itoa(len(bundle.LocalResponseAppealAutomation)),
			bundle.AlignmentResponseBundleHash,
			joinSecureCellFederationControlIDs(bundle.Controls),
			joinSecureCellFederationOperatorSurfaceIDs(bundle.OperatorSurfaces),
			joinSecureCellFederationOperatorSurfacePaths(bundle.OperatorSurfaces),
			bundle.ControlLedgerID,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"appeal_review_id", "review_id", "cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "local_ruling",
			"counterparty_snapshot_id", "counterparty_bundle_id", "counterparty_bundle_status", "counterparty_response_appeal_id", "counterparty_response_appeal_status", "counterparty_response_status", "counterparty_response_action", "counterparty_ruling",
			"review_status", "latest_review_action", "review_action_count", "review_last_reviewed_at",
			"board_response_appeal_id", "board_parent_response_appeal_id", "board_response_appeal_generation", "board_response_appeal_status", "board_response_status", "board_response_action",
			"board_review_threshold", "board_delegation_count", "board_recusal_count", "board_committee_member_count", "board_recorded_vote_count", "board_outstanding_votes", "board_missing_quorum_count", "board_threshold_satisfied",
			"board_ratify_vote_count", "board_overturn_vote_count", "board_ruling", "board_ruling_summary", "board_action_count", "board_automation_action_count",
			"created_at", "updated_at", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed", "portable_package_anchored",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				string(item.LatestReviewAction),
				strconv.Itoa(item.ReviewActionCount),
				secureCellCSVTime(item.ReviewLastReviewedAt),
				item.BoardResponseAppealID,
				item.BoardParentResponseAppealID,
				strconv.Itoa(item.BoardResponseAppealGeneration),
				string(item.BoardResponseAppealStatus),
				string(item.BoardResponseStatus),
				string(item.BoardResponseAction),
				strconv.Itoa(item.BoardReviewThreshold),
				strconv.Itoa(item.BoardDelegationCount),
				strconv.Itoa(item.BoardRecusalCount),
				strconv.Itoa(item.BoardCommitteeMemberCount),
				...
				strconv.Itoa(item.BoardMissingQuorumCount),
				strconv.FormatBool(item.BoardThresholdSatisfied),
				strconv.Itoa(item.BoardRatifyVoteCount),
				strconv.Itoa(item.BoardOverturnVoteCount),
				string(item.BoardRuling),
				item.BoardRulingSummary,
				strconv.Itoa(item.BoardActionCount),
				strconv.Itoa(item.BoardAutomationActionCount),
				item.CreatedAt.UTC().Format(timeCSVFormat),
				item.UpdatedAt.UTC().Format(timeCSVFormat),
				item.ControlLedgerID,
				item.ControlLedgerHash,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response appeal counterparty review appeal bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"appeal_review_id", "review_id", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "counterparty_snapshot_id", "counterparty_response_appeal_id", "review_status", "latest_review_action",
			"board_response_appeal_id", "board_response_appeal_status", "board_ruling", "board_review_threshold", "board_action_count", "board_automation_action_count",
			"counterparty_action_count", "local_board_response_action_count", "local_board_response_recusal_count", "local_board_response_automation_action_count",
			"counterparty_review_bundle_hash", "alignment_response_bundle_hash", "challenge_appeal_bundle_hash",
			"control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash",
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
			return err
		...
		if err := cw.Write([]string{
			...
			bundle.Organization.OrganizationName,
			string(bundle.Organization.Status),
			bundle.ReviewAppeal.AppealReviewID,
			bundle.ReviewAppeal.ReviewID,
			bundle.ReviewAppeal.IncidentID,
			bundle.ReviewAppeal.ResponseID,
			bundle.ReviewAppeal.DirectiveID,
			bundle.ReviewAppeal.ExtensionID,
			bundle.ReviewAppeal.DisputeID,
			bundle.ReviewAppeal.AppealID,
			bundle.ReviewAppeal.ChallengeID,
			bundle.ReviewAppeal.ChallengeAppealID,
			bundle.ReviewAppeal.ResponseAppealID,
			bundle.ReviewAppeal.CounterpartySnapshotID,
			bundle.ReviewAppeal.CounterpartyResponseAppealID,
			string(bundle.ReviewAppeal.ReviewStatus),
			string(bundle.ReviewAppeal.LatestReviewAction),
			bundle.ReviewAppeal.BoardResponseAppealID,
			string(bundle.ReviewAppeal.BoardResponseAppealStatus),
			string(bundle.ReviewAppeal.BoardRuling),
			strconv.Itoa(bundle.ReviewAppeal.BoardReviewThreshold),
			strconv.Itoa(bundle.ReviewAppeal.BoardActionCount),
			strconv.Itoa(bundle.ReviewAppeal.BoardAutomationActionCount),
			strconv.Itoa(len(bundle.CounterpartyActions)),
			strconv.Itoa(len(bundle.LocalBoardResponseActions)),
			strconv.Itoa(len(bundle.LocalBoardResponseRecusals)),
			strconv.Itoa(len(bundle.LocalBoardResponseAutomation)),
			bundle.CounterpartyReviewBundleHash,
			bundle.AlignmentResponseBundleHash,
			bundle.ChallengeAppealBundleHash,
			joinSecureCellFederationControlIDs(bundle.Controls),
			joinSecureCellFederationOperatorSurfaceIDs(bundle.OperatorSurfaces),
...
func writeSecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-counterparty-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"snapshot_id", "bundle_id", "bundle_version", "bundle_name", "status", "verified", "verification_message", "signer", "key_id",
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "local_ruling",
			"counterparty_review_id", "counterparty_review_appeal_id", "counterparty_review_status", "counterparty_review_action", "counterparty_review_reference",
			"counterparty_board_response_appeal_id", "counterparty_board_response_appeal_status", "counterparty_board_response_status", "counterparty_board_response_action", "counterparty_board_ruling",
			"alignment_status", "alignment_divergence_count", "review_status", "last_reviewed_by", "last_reviewed_at", "review_action_count",
			"generated_at", "expires_at", "received_at", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed", "portable_package_anchored",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				string(item.ResponseAction),
				string(item.LocalRuling),
				item.CounterpartyReviewID,
				item.CounterpartyReviewAppealID,
				string(item.CounterpartyReviewStatus),
				string(item.CounterpartyReviewAction),
				item.CounterpartyReviewReference,
				item.CounterpartyBoardResponseAppealID,
				string(item.CounterpartyBoardResponseAppealStatus),
				string(item.CounterpartyBoardResponseStatus),
				string(item.CounterpartyBoardResponseAction),
				string(item.CounterpartyBoardRuling),
				string(item.AlignmentStatus),
				strconv.Itoa(item.AlignmentDivergenceCount),
				string(item.ReviewStatus),
				item.LastReviewedBy,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"transition_id", "action", "review_status", "status", "actor_did", "reason", "occurred_at",
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "local_ruling",
			"snapshot_id", "bundle_id", "counterparty_review_id", "counterparty_review_appeal_id", "counterparty_review_status", "counterparty_review_action", "counterparty_review_reference",
			"counterparty_board_response_appeal_id", "counterparty_board_response_appeal_status", "counterparty_board_response_status", "counterparty_board_response_action", "counterparty_board_ruling",
			"alignment_status", "alignment_divergence_count", "divergences", "policy_receipt_id", "policy_receipt_hash", "seal_id", "trace_link_id", "metadata",
		}); err != nil {
			return err
		}
		for _, item := range items {
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response appeal counterparty review appeal review bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"snapshot_id", "bundle_id", "bundle_version", "bundle_name", "review_status", "review_action_count", "review_last_reviewed_at",
			"latest_local_review_action", "local_review_action_types",
			"counterparty_review_id", "counterparty_review_appeal_id", "counterparty_review_status", "counterparty_review_action", "counterparty_review_reference",
			"counterparty_board_response_appeal_id", "counterparty_board_response_appeal_status", "counterparty_board_response_status", "counterparty_board_response_action", "counterparty_board_ruling",
			"local_board_response_appeal_id", "local_board_response_status", "local_board_response_action", "local_board_ruling",
			"review_actions_count", "local_board_response_actions_count", "local_board_response_recusals_count", "local_board_response_automation_action_count",
			"counterparty_review_appeal_bundle_hash", "counterparty_review_bundle_hash", "alignment_response_bundle_hash", "challenge_appeal_bundle_hash",
			"control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash",
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
		...
		localBoardResponseAppealID := ""
		localBoardResponseStatus := ""
		localBoardResponseAction := ""
		localBoardResponseRuling := ""
		latestLocalReviewAction := ""
		localReviewActionTypes := make([]string, 0, len(bundle.ReviewActions))
		if len(bundle.ReviewActions) > 0 {
			latestLocalReviewAction = string(bundle.ReviewActions[0].Action)
			for _, item := range bundle.ReviewActions {
				localReviewActionTypes = append(localReviewActionTypes, string(item.Action))
			}
		}
		if bundle.LocalBoardResponseAppeal != nil {
			localBoardResponseAppealID = bundle.LocalBoardResponseAppeal.ResponseAppealID
			localBoardResponseStatus = string(bundle.LocalBoardResponseAppeal.Status)
			localBoardResponseAction = string(bundle.LocalBoardResponseAppeal.ResponseAction)
			localBoardResponseRuling = string(bundle.LocalBoardResponseAppeal.Ruling)
		}
		if err := cw.Write([]string{
			bundle.ID,
			bundle.Version,
			...
			bundle.Organization.OrganizationName,
			string(bundle.Organization.Status),
			bundle.Review.SnapshotID,
			bundle.Review.BundleID,
			bundle.Review.BundleVersion,
			bundle.Review.BundleName,
			string(bundle.Review.ReviewStatus),
			strconv.Itoa(bundle.Review.ReviewActionCount),
			secureCellCSVTime(bundle.Review.LastReviewedAt),
			latestLocalReviewAction,
			strings.Join(localReviewActionTypes, "|"),
			bundle.Review.CounterpartyReviewID,
			bundle.Review.CounterpartyReviewAppealID,
			string(bundle.Review.CounterpartyReviewStatus),
			string(bundle.Review.CounterpartyReviewAction),
			bundle.Review.CounterpartyReviewReference,
			bundle.Review.CounterpartyBoardResponseAppealID,
			string(bundle.Review.CounterpartyBoardResponseAppealStatus),
			string(bundle.Review.CounterpartyBoardResponseStatus),
			string(bundle.Review.CounterpartyBoardResponseAction),
			string(bundle.Review.CounterpartyBoardRuling),
			localBoardResponseAppealID,
			localBoardResponseStatus,
			localBoardResponseAction,
			localBoardResponseRuling,
			strconv.Itoa(len(bundle.ReviewActions)),
			strconv.Itoa(len(bundle.LocalBoardResponseActions)),
			strconv.Itoa(len(bundle.LocalBoardResponseRecusals)),
			strconv.Itoa(len(bundle.LocalBoardResponseAutomation)),
			bundle.CounterpartyReviewAppealBundleHash,
			bundle.CounterpartyReviewBundleHash,
			bundle.AlignmentResponseBundleHash,
			bundle.ChallengeAppealBundleHash,
			joinSecureCellFederationControlIDs(bundle.Controls),
...
func writeSecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-counterparty-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"snapshot_id", "bundle_id", "bundle_version", "bundle_name", "status", "verified", "verification_message", "signer",
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "response_transition_id", "ruling",
			"review_snapshot_id", "review_bundle_id", "review_bundle_status", "review_status", "latest_review_action", "review_action_count",
			"counterparty_review_id", "counterparty_review_appeal_id",
			"prior_board_response_appeal_id", "prior_board_response_appeal_status", "prior_board_response_status", "prior_board_response_action",
			"generated_at", "expires_at", "received_at", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash", "received_by",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.SponsorOfRecord,
				item.OrganizationName,
				string(item.OrganizationStatus),
				item.IncidentID,
				item.ResponseID,
				item.DirectiveID,
				item.ExtensionID,
				...
				item.ReviewBundleID,
				string(item.ReviewBundleStatus),
				string(item.ReviewStatus),
				string(item.LatestReviewAction),
				strconv.Itoa(item.ReviewActionCount),
				item.CounterpartyReviewID,
				item.CounterpartyReviewAppealID,
				item.PriorBoardResponseAppealID,
				string(item.PriorBoardResponseAppealStatus),
				string(item.PriorBoardResponseStatus),
				string(item.PriorBoardResponseAction),
				item.GeneratedAt.UTC().Format(timeCSVFormat),
				secureCellCSVTime(item.ExpiresAt),
				item.ReceivedAt.UTC().Format(timeCSVFormat),
				item.ControlLedgerID,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "ruling",
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"review_snapshot_id", "review_bundle_id", "review_bundle_status", "review_bundle_verified", "review_signer",
			"counterparty_review_id", "counterparty_review_appeal_id", "review_status", "latest_review_action", "review_action_count", "review_last_reviewed_at",
			"prior_board_response_appeal_id", "prior_board_response_appeal_status", "prior_board_response_status", "prior_board_response_action", "prior_board_ruling",
			"board_review_threshold", "board_recorded_vote_count", "board_missing_quorum_count", "board_threshold_satisfied",
			"action_count", "automation_action_count", "control_ledger_id", "control_ledger_hash", "portable_package_hash", "portable_package_signed", "portable_package_anchored", "created_at", "updated_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.ReviewBundleID,
				string(item.ReviewBundleStatus),
				strconv.FormatBool(item.ReviewBundleVerified),
				item.ReviewSigner,
				item.CounterpartyReviewID,
				item.CounterpartyReviewAppealID,
				string(item.ReviewStatus),
				...
				string(item.PriorBoardResponseStatus),
				string(item.PriorBoardResponseAction),
				string(item.PriorBoardRuling),
				strconv.Itoa(item.BoardReviewThreshold),
				strconv.Itoa(item.BoardRecordedVoteCount),
				strconv.Itoa(item.BoardMissingQuorumCount),
				strconv.FormatBool(item.BoardThresholdSatisfied),
				strconv.Itoa(item.ActionCount),
				strconv.Itoa(item.AutomationActionCount),
				item.ControlLedgerID,
				item.ControlLedgerHash,
				item.PortablePackageHash,
				strconv.FormatBool(item.PortablePackageSigned),
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeal-review-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "ruling",
			"snapshot_id", "bundle_id", "review_snapshot_id", "review_bundle_id", "imported_status",
			"local_review_status", "action", "actor_did", "counterparty_reference", "divergences",
			"transition_id", "policy_receipt_id", "policy_receipt_hash", "seal_id", "trace_link_id", "reason", "metadata", "occurred_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.ReviewSnapshotID,
				item.ReviewBundleID,
				string(item.ImportedStatus),
				string(item.LocalReviewStatus),
				string(item.Action),
				item.ActorDID,
				item.CounterpartyReference,
				strings.Join(item.Divergences, "|"),
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundleExport(w http.ResponseWriter, r *http.Request, snapshotID string, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response appeal counterparty review appeal review appeal bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeal-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"snapshot_id", "id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"review_snapshot_id", "review_bundle_id", "review_bundle_status", "review_status", "latest_review_action", "review_action_count",
			"prior_board_response_appeal_id", "prior_board_response_appeal_status", "prior_board_response_status", "prior_board_response_action", "prior_board_ruling",
			"response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "ruling",
			"review_actions_count", "prior_board_response_actions_count", "prior_board_response_recusals_count", "prior_board_response_automation_action_count",
			"review_appeal_actions_count", "review_appeal_recusals_count", "review_appeal_automation_action_count",
			"counterparty_review_appeal_review_bundle_hash", "counterparty_review_appeal_bundle_hash", "counterparty_review_bundle_hash", "alignment_response_bundle_hash", "challenge_appeal_bundle_hash",
			"control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash",
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
		...
		priorBoardResponseAppealID := ""
		priorBoardResponseStatus := ""
		priorBoardResponseAction := ""
		priorBoardResponseRuling := ""
		if bundle.PriorBoardResponseAppeal != nil {
			priorBoardResponseAppealID = bundle.PriorBoardResponseAppeal.ResponseAppealID
			priorBoardResponseStatus = string(bundle.PriorBoardResponseAppeal.Status)
			priorBoardResponseAction = string(bundle.PriorBoardResponseAppeal.ResponseAction)
			priorBoardResponseRuling = string(bundle.PriorBoardResponseAppeal.Ruling)
		}
		if err := cw.Write([]string{
			snapshotID,
			bundle.ID,
			...
			bundle.Organization.OrganizationName,
			string(bundle.Organization.Status),
			bundle.ReviewAppeal.ReviewSnapshotID,
			bundle.ReviewAppeal.ReviewBundleID,
			string(bundle.ReviewAppeal.ReviewBundleStatus),
			string(bundle.ReviewAppeal.ReviewStatus),
			string(bundle.ReviewAppeal.LatestReviewAction),
			strconv.Itoa(bundle.ReviewAppeal.ReviewActionCount),
			priorBoardResponseAppealID,
			string(bundle.ReviewAppeal.PriorBoardResponseAppealStatus),
			priorBoardResponseStatus,
			priorBoardResponseAction,
			priorBoardResponseRuling,
			bundle.ReviewAppeal.ResponseAppealID,
			strconv.Itoa(bundle.ReviewAppeal.ResponseAppealGeneration),
			string(bundle.ReviewAppeal.ResponseAppealStatus),
			string(bundle.ReviewAppeal.ResponseStatus),
			string(bundle.ReviewAppeal.ResponseAction),
			string(bundle.ReviewAppeal.Ruling),
			strconv.Itoa(len(bundle.ReviewActions)),
			strconv.Itoa(len(bundle.PriorBoardResponseActions)),
			strconv.Itoa(len(bundle.PriorBoardResponseRecusals)),
			strconv.Itoa(len(bundle.PriorBoardResponseAutomation)),
			strconv.Itoa(len(bundle.ReviewAppealActions)),
			strconv.Itoa(len(bundle.ReviewAppealRecusals)),
			strconv.Itoa(len(bundle.ReviewAppealAutomation)),
			bundle.CounterpartyReviewAppealReviewBundleHash,
			bundle.CounterpartyReviewAppealBundleHash,
			bundle.CounterpartyReviewBundleHash,
			bundle.AlignmentResponseBundleHash,
			bundle.ChallengeAppealBundleHash,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal alignment response appeal counterparty review appeal review appeal review bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeal-review-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"snapshot_id", "bundle_id", "bundle_status", "review_status", "latest_review_action", "review_action_count",
			"response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "ruling",
			"local_board_response_appeal_id", "local_board_response_appeal_status", "local_board_response_status", "local_board_response_action", "local_board_response_ruling",
			"review_actions_count", "local_board_response_actions_count", "local_board_response_recusals_count", "local_board_response_automation_action_count",
			"counterparty_review_appeal_review_appeal_bundle_hash", "counterparty_review_appeal_review_bundle_hash", "counterparty_review_appeal_bundle_hash", "counterparty_review_bundle_hash", "alignment_response_bundle_hash", "challenge_appeal_bundle_hash",
			"control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash",
			"signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
		...
		localBoardResponseAppealID := ""
		localBoardResponseAppealStatus := ""
		localBoardResponseStatus := ""
		localBoardResponseAction := ""
		localBoardResponseRuling := ""
		if bundle.LocalBoardResponseAppeal != nil {
			localBoardResponseAppealID = bundle.LocalBoardResponseAppeal.ResponseAppealID
			localBoardResponseAppealStatus = string(bundle.LocalBoardResponseAppeal.Status)
			localBoardResponseStatus = string(bundle.LocalBoardResponseAppeal.ResponseStatus)
			localBoardResponseAction = string(bundle.LocalBoardResponseAppeal.ResponseAction)
			localBoardResponseRuling = string(bundle.LocalBoardResponseAppeal.Ruling)
		}
		if err := cw.Write([]string{
			...
			bundle.Review.SnapshotID,
			bundle.Review.BundleID,
			string(bundle.Review.Status),
			string(bundle.Review.ReviewStatus),
			string(bundle.Review.LatestReviewAction),
			strconv.Itoa(bundle.Review.ReviewActionCount),
			bundle.Review.ResponseAppealID,
			strconv.Itoa(bundle.Review.ResponseAppealGeneration),
			string(bundle.Review.ResponseAppealStatus),
			string(bundle.Review.ResponseStatus),
			string(bundle.Review.ResponseAction),
			string(bundle.Review.Ruling),
			localBoardResponseAppealID,
			localBoardResponseAppealStatus,
			localBoardResponseStatus,
			localBoardResponseAction,
			...
			strconv.Itoa(len(bundle.LocalBoardResponseRecusals)),
			strconv.Itoa(len(bundle.LocalBoardResponseAutomation)),
			bundle.CounterpartyReviewAppealReviewAppealBundleHash,
			bundle.CounterpartyReviewAppealReviewBundleHash,
			bundle.CounterpartyReviewAppealBundleHash,
			bundle.CounterpartyReviewBundleHash,
			bundle.AlignmentResponseBundleHash,
...
func writeSecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-counterparty-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeal-review-bundles.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"snapshot_id", "bundle_id", "bundle_version", "bundle_name", "status", "verified", "verification_message", "signer",
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_generation", "response_appeal_status", "response_status", "response_action", "response_transition_id", "ruling",
			"reviewed_snapshot_id", "reviewed_bundle_id", "reviewed_bundle_status", "reviewed_review_status", "latest_reviewed_action", "reviewed_action_count",
			"counterparty_review_id", "counterparty_review_appeal_id",
			"local_board_response_appeal_id", "local_board_response_appeal_status", "local_board_response_status", "local_board_response_action",
			"generated_at", "expires_at", "received_at", "control_ledger_id", "control_ledger_hash",
			"portable_package_hash", "portable_package_signed", "portable_package_anchored", "content_hash", "received_by", "metadata",
		}); err != nil {
			return err
		}
		for _, item := range items {
			if err := cw.Write([]string{
				...
				item.ResponseTransitionID,
				string(item.Ruling),
				item.ReviewedSnapshotID,
				item.ReviewedBundleID,
				string(item.ReviewedBundleStatus),
				string(item.ReviewedReviewStatus),
				string(item.LatestReviewedAction),
				strconv.Itoa(item.ReviewedActionCount),
				item.CounterpartyReviewID,
				item.CounterpartyReviewAppealID,
				item.LocalBoardResponseAppealID,
				string(item.LocalBoardResponseAppealStatus),
				string(item.LocalBoardResponseStatus),
				string(item.LocalBoardResponseAction),
				item.GeneratedAt.UTC().Format(timeCSVFormat),
				secureCellCSVTime(item.ExpiresAt),
				item.ReceivedAt.UTC().Format(timeCSVFormat),
				item.ControlLedgerID,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewRecord) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-counterparty-review-appeal-review-appeal-review-bundle-review-actions.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "cell_status", "jurisdiction", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "response_appeal_status", "response_status", "response_action", "ruling",
			"snapshot_id", "bundle_id", "reviewed_snapshot_id", "reviewed_bundle_id", "imported_status",
			"local_review_status", "action", "actor_did", "counterparty_reference", "divergences",
			"transition_id", "policy_receipt_id", "policy_receipt_hash", "seal_id", "trace_link_id", "reason", "metadata", "occurred_at",
		}); err != nil {
			return err
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleExport(w http.ResponseWriter, r *http.Request, bundle *securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		if bundle == nil {
			return fmt.Errorf("federation incident directive extension appeal reconciliation challenge appeal bundle is required")
		}
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-bundle.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"id", "version", "name", "generated_at", "expires_at", "cell_id", "cell_name", "cell_status", "jurisdiction", "framework",
			"organization_id", "sponsor_of_record", "organization_name", "organization_status",
			"comparison_key", "incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id",
			"challenge_status", "challenge_ruling", "challenge_appeal_id", "parent_challenge_appeal_id", "challenge_appeal_generation", "challenge_appeal_status",
			"appealing_party", "board_party", "enforcement_acknowledgement_party", "board_review_threshold", "board_delegation_count", "board_recusal_count",
			"board_committee_member_count", "board_recorded_vote_count", "board_outstanding_votes", "board_missing_quorum_count", "board_threshold_satisfied",
			"ratify_vote_count", "overturn_vote_count", "ruling", "action_count", "counterparty_alignment_action_count", "counterparty_alignment_automation_action_count", "counterparty_alignment_response_action_count", "counterparty_alignment_response_appeal_action_count", "counterparty_alignment_response_appeal_automation_action_count", "recusal_count", "automation_action_count", "reconciliation_bundle_hash",
			"control_ids", "operator_surface_ids", "operator_surface_paths", "control_ledger_id", "control_ledger_hash", "portable_package_hash",
			"portable_package_signed", "portable_package_anchored", "content_hash", "signature_algorithm", "signature_signer", "signature_key_id", "signature_signed_at", "metadata",
		}); err != nil {
			return err
		}
		signatureAlgorithm := ""
		...
		if err := cw.Write([]string{
			...
			bundle.ChallengeAppealSummary.DisputeID,
			bundle.ChallengeAppealSummary.AppealID,
			bundle.ChallengeSummary.ChallengeID,
			string(bundle.ChallengeSummary.ChallengeStatus),
			string(bundle.ChallengeSummary.Ruling),
			bundle.ChallengeAppealSummary.ChallengeAppealID,
			bundle.ChallengeAppealSummary.ParentChallengeAppealID,
			strconv.Itoa(bundle.ChallengeAppealSummary.ChallengeAppealGeneration),
			string(bundle.ChallengeAppealSummary.ChallengeAppealStatus),
			string(bundle.ChallengeAppealSummary.AppealingParty),
			string(bundle.ChallengeAppealSummary.BoardParty),
			string(bundle.ChallengeAppealSummary.EnforcementAcknowledgementParty),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardReviewThreshold),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardDelegationCount),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardRecusalCount),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardCommitteeMemberCount),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardRecordedVoteCount),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardOutstandingVotes),
			strconv.Itoa(bundle.ChallengeAppealSummary.BoardMissingQuorumCount),
			strconv.FormatBool(bundle.ChallengeAppealSummary.BoardThresholdSatisfied),
			strconv.Itoa(bundle.ChallengeAppealSummary.RatifyVoteCount),
			strconv.Itoa(bundle.ChallengeAppealSummary.OverturnVoteCount),
			string(bundle.ChallengeAppealSummary.Ruling),
			strconv.Itoa(len(bundle.Actions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentActions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentAutomationActions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentResponseActions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentResponseAppealActions)),
			strconv.Itoa(len(bundle.CounterpartyAlignmentResponseAppealAutomationActions)),
			strconv.Itoa(len(bundle.Recusals)),
			strconv.Itoa(len(bundle.AutomationActions)),
			bundle.ReconciliationBundleHash,
			joinSecureCellFederationControlIDs(bundle.Controls),
			joinSecureCellFederationOperatorSurfaceIDs(bundle.OperatorSurfaces),
			joinSecureCellFederationOperatorSurfacePaths(bundle.OperatorSurfaces),
			bundle.ControlLedgerID,
...
func writeSecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecusalExport(w http.ResponseWriter, r *http.Request, items []securecellsintegration.SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecusalSummary) error {
	format := secureCellExportFormat(r)
	switch format {
	case "json":
	...
	case "csv":
		w.Header().Set("Content-Type", "text/csv; charset=utf-8")
		w.Header().Set("Content-Disposition", `attachment; filename="secure-cell-federation-incident-directive-extension-appeal-reconciliation-challenge-appeal-alignment-response-appeal-recusals.csv"`)
		cw := csv.NewWriter(w)
		if err := cw.Write([]string{
			"cell_id", "cell_name", "jurisdiction", "cell_status", "organization_id", "sponsor_of_record", "organization_name",
			"incident_id", "response_id", "directive_id", "extension_id", "dispute_id", "appeal_id", "challenge_id", "challenge_appeal_id",
			"response_appeal_id", "parent_response_appeal_id", "response_appeal_generation", "response_status", "response_appeal_status",
			"correction_board_party", "recusal_id", "actor_did", "summary", "description", "created_at",
		}); err != nil {
			return err
		}
		for _, item := range items {
