...
mod tests {
    ...
    fn setup_job(deps: DepsMut, creator: &str, payment: u128) -> (String, MessageInfo) {
        ...
        let job_id = res.attributes.iter()
            .find(|a| a.key == "job_id")
            .map(|a| a.value.clone())
            .unwrap();
    ...
    fn mock_tee_attestation() -> TEEAttestation {
        TEEAttestation {
            ...
            measurement: "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2".to_string(),
    ...
    fn assign_job_works() {
        ...
        let msg = ExecuteMsg::AssignJob { job_id: job_id.clone() };
        
        let res = execute(deps.as_mut(), mock_env(), info.clone(), msg).unwrap();
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "assign_job"));
    ...
    fn assign_job_not_pending_fails() {
        ...
        // First assignment
        let info = mock_info(VALIDATOR, &[]);
        execute(deps.as_mut(), mock_env(), info.clone(), ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
        
        // Second assignment should fail
        let err = execute(deps.as_mut(), mock_env(), info, ExecuteMsg::AssignJob { job_id }).unwrap_err();
    ...
    fn start_computing_works() {
        ...
        // Start computing
        let msg = ExecuteMsg::StartComputing { job_id: job_id.clone() };
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "start_computing"));
    ...
    fn start_computing_not_assigned_validator_fails() {
        ...
        execute(deps.as_mut(), mock_env(), info1, ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
    ...
    fn complete_job_works() {
        ...
        let info = mock_info(VALIDATOR, &[]);
        execute(deps.as_mut(), mock_env(), info.clone(), ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), info.clone(), ExecuteMsg::StartComputing { job_id: job_id.clone() }).unwrap();
    ...
    fn verify_job_works() {
        ...
        execute(deps.as_mut(), mock_env(), validator_info.clone(), ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), validator_info.clone(), ExecuteMsg::StartComputing { job_id: job_id.clone() }).unwrap();
        
        let tee_attestation = mock_tee_attestation();
        let compute_metrics = mock_compute_metrics();
        
        execute(deps.as_mut(), mock_env(), validator_info, ExecuteMsg::CompleteJob {
            job_id: job_id.clone(),
            output_hash: "output789".to_string(),
            tee_attestation,
            compute_metrics,
        ...
        let msg = ExecuteMsg::VerifyJob { job_id: job_id.clone() };
        let res = execute(deps.as_mut(), mock_env(), creator_info, msg).unwrap();
        
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "verify_job"));
    ...
    fn fail_job_works() {
        ...
        // Fail job
        let msg = ExecuteMsg::FailJob { job_id: job_id.clone(), reason: "Out of memory".to_string() };
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "fail_job"));
        assert!(res.attributes.iter().any(|a| a.key == "reason" && a.value == "Out of memory"));
    ...
    fn cancel_job_works() {
        ...
        let msg = ExecuteMsg::CancelJob { job_id: job_id.clone() };
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "cancel_job"));
    ...
    fn cancel_job_not_pending_fails() {
        ...
        // Assign job first
        let validator_info = mock_info(VALIDATOR, &[]);
        execute(deps.as_mut(), mock_env(), validator_info, ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
    ...
    fn claim_payment_works() {
        ...
        execute(deps.as_mut(), mock_env(), validator_info.clone(), ExecuteMsg::CompleteJob {
            job_id: job_id.clone(),
            output_hash: "output789".to_string(),
            tee_attestation,
            compute_metrics,
        ...
        let creator_info = mock_info(CREATOR, &[]);
        execute(deps.as_mut(), mock_env(), creator_info, ExecuteMsg::VerifyJob { job_id: job_id.clone() }).unwrap();
        
        // Claim payment
        let msg = ExecuteMsg::ClaimPayment { job_id: job_id.clone() };
    ...
    fn query_job_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::Job { job_id: job_id.clone() }).unwrap();
    ...
    fn query_job_not_found_fails() {
        ...
        let err = query(deps.as_ref(), mock_env(), QueryMsg::Job { job_id: "nonexistent".to_string() }).unwrap_err();
    ...
    fn query_list_jobs_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::ListJobs {
            status: None,
    ...
    fn query_pending_queue_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::PendingQueue { limit: Some(10) }).unwrap();
        let jobs: Vec<Job> = from_json(&res).unwrap();
    ...
    fn query_pricing_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::EstimatePrice {
            model_hash: "model123".to_string(),
    ...
    fn validator_stats_updated_on_complete() {
        ...
        let stats = VALIDATOR_STATS.load(&deps.storage, &Addr::unchecked(VALIDATOR)).unwrap();
    ...
    fn cleanup_expired_works() {
        ...
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "cleanup_expired"));
    ...
    fn platform_stats_completed_jobs_increments_on_verify() {
        ...
        let validator_info = mock_info(VALIDATOR, &[]);
        execute(deps.as_mut(), mock_env(), validator_info.clone(),
            ExecuteMsg::AssignJob { job_id: job_id.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), validator_info.clone(),
            ExecuteMsg::StartComputing { job_id: job_id.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), validator_info, ExecuteMsg::CompleteJob {
            job_id: job_id.clone(),
            output_hash: "output789".to_string(),
        ...
        // Verify
        let creator_info = mock_info(CREATOR, &[]);
        execute(deps.as_mut(), mock_env(), creator_info,
            ExecuteMsg::VerifyJob { job_id }).unwrap();
    ...
    fn platform_stats_failed_jobs_increments_on_fail() {
        ...
        // Fail
        execute(deps.as_mut(), mock_env(), validator_info,
            ExecuteMsg::FailJob { job_id: job_id.clone(), reason: "OOM".to_string() }).unwrap();
    ...
    fn platform_stats_total_payments_increments_on_claim() {
        ...
        let creator_info = mock_info(CREATOR, &[]);
        execute(deps.as_mut(), mock_env(), creator_info,
            ExecuteMsg::VerifyJob { job_id: job_id.clone() }).unwrap();
        ...
        // Claim payment
        execute(deps.as_mut(), mock_env(), mock_info(VALIDATOR, &[]),
            ExecuteMsg::ClaimPayment { job_id }).unwrap();
    ...
    fn platform_stats_aggregates_multiple_operations() {
        ...
        execute(deps.as_mut(), mock_env(), v_info.clone(),
            ExecuteMsg::AssignJob { job_id: job_id1.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), v_info.clone(),
            ExecuteMsg::StartComputing { job_id: job_id1.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), v_info.clone(), ExecuteMsg::CompleteJob {
        ...
        execute(deps.as_mut(), mock_env(), mock_info(CREATOR, &[]),
            ExecuteMsg::VerifyJob { job_id: job_id1 }).unwrap();

        // Job 2: fail
        let (job_id2, _) = setup_job(deps.as_mut(), CREATOR, 10000);
        execute(deps.as_mut(), mock_env(), v_info.clone(),
            ExecuteMsg::AssignJob { job_id: job_id2.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), v_info.clone(),
            ExecuteMsg::StartComputing { job_id: job_id2.clone() }).unwrap();
        execute(deps.as_mut(), mock_env(), v_info,
            ExecuteMsg::FailJob { job_id: job_id2, reason: "Error".to_string() }).unwrap();
    ...
    fn double_claim_payment_rejected() {
        ...
        execute(deps.as_mut(), mock_env(), validator_info.clone(),
            ExecuteMsg::ClaimPayment { job_id: job_id.clone() }).unwrap();

        // Second claim fails with AlreadyClaimed
        let err = execute(deps.as_mut(), mock_env(), validator_info,
            ExecuteMsg::ClaimPayment { job_id }).unwrap_err();
    ...
    fn update_config_fee_above_cap_rejected() {
        ...
        let err = execute(deps.as_mut(), mock_env(), admin_info, ExecuteMsg::UpdateConfig {
            min_payment: None,
    ...
    fn update_config_fee_at_cap_succeeds() {
        ...
        let admin_info = mock_info(ADMIN, &[]);
        execute(deps.as_mut(), mock_env(), admin_info, ExecuteMsg::UpdateConfig {
            min_payment: None,
    ...
    fn verify_job_emits_monitoring_event() {
        ...
        let creator_info = mock_info(CREATOR, &[]);
        let res = execute(deps.as_mut(), mock_env(), creator_info,
            ExecuteMsg::VerifyJob { job_id }).unwrap();

        let event = res.events.iter().find(|e| e.ty == "job_verified");
        assert!(event.is_some(), "job_verified event should be emitted");
        let ev = event.unwrap();
        assert!(ev.attributes.iter().any(|a| a.key == "creator" && a.value == CREATOR));
        assert!(ev.attributes.iter().any(|a| a.key == "validator" && a.value == VALIDATOR));
    }
    ...
    fn fail_job_emits_monitoring_event() {
        ...
        let res = execute(deps.as_mut(), mock_env(), validator_info,
            ExecuteMsg::FailJob { job_id, reason: "OOM".to_string() }).unwrap();

        let event = res.events.iter().find(|e| e.ty == "job_failed");
        assert!(event.is_some(), "job_failed event should be emitted");
        let ev = event.unwrap();
        assert!(ev.attributes.iter().any(|a| a.key == "reason" && a.value == "OOM"));
    }
