...
mod tests {
    ...
    use cosmwasm_std::{from_json, to_json_binary, Addr, Timestamp, Uint128,
        SystemResult, ContractResult, OwnedDeps, MemoryStorage,
    };
    ...
    fn create_seal(deps: DepsMut, requester: &str, validators: Vec<&str>) -> (String, MessageInfo) {
        ...
        let seal_id = res.attributes.iter()
    ...
    fn create_seal_works() {
        ...
        let (seal_id, _) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
    ...
    fn revoke_seal_works() {
        let mut deps = mock_deps_with_wasm();
        setup_contract(deps.as_mut());
        let (seal_id, requester_info) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        ...
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "revoke_seal"));
    ...
    fn revoke_seal_not_active_fails() {
        ...
        // Revoke once
        execute(deps.as_mut(), mock_env(), requester_info.clone(), ExecuteMsg::RevokeSeal {
            seal_id: seal_id.clone(),
    ...
    fn verify_active_seal_works() {
        ...
        let msg = ExecuteMsg::VerifySeal { seal_id: seal_id.clone() };
        let res = execute(deps.as_mut(), mock_env(), mock_info("anyone", &[]), msg).unwrap();

        // C-02 FIX: VerifySeal now performs real validation
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "verify_seal"));
        assert!(res.attributes.iter().any(|a| a.key == "valid" && a.value == "true"));
        assert!(res.attributes.iter().any(|a| a.key == "status" && a.value == "active"));
        assert!(res.attributes.iter().any(|a| a.key == "validators" && a.value == "3"));
    }
    ...
    fn verify_revoked_seal_returns_invalid() {
        ...
        // Revoke seal
        execute(deps.as_mut(), mock_env(), requester_info, ExecuteMsg::RevokeSeal {
            seal_id: seal_id.clone(),
            reason: "Test revoke".to_string(),
        }).unwrap();

        // VerifySeal should return valid=false
        let res = execute(deps.as_mut(), mock_env(), mock_info("anyone", &[]), ExecuteMsg::VerifySeal {
            seal_id: seal_id.clone(),
        }).unwrap();
        assert!(res.attributes.iter().any(|a| a.key == "valid" && a.value == "false"));
        assert!(res.attributes.iter().any(|a| a.key == "status" && a.value == "revoked"));
    }
    ...
    fn verify_nonexistent_seal_fails() {
        ...
        let err = execute(deps.as_mut(), mock_env(), mock_info("anyone", &[]),
            ExecuteMsg::VerifySeal { seal_id: "nonexistent".to_string() },
        ).unwrap_err();
    ...
    fn batch_verify_returns_accurate_counts() {
        ...
        let (seal_id1, requester_info) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        let (seal_id2, _) = create_seal(deps.as_mut(), "requester2", vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        ...
        let res = execute(deps.as_mut(), mock_env(), mock_info("anyone", &[]),
            ExecuteMsg::BatchVerify { seal_ids: vec![seal_id1, seal_id2, "nonexistent".to_string()] },
        ).unwrap();

        assert!(res.attributes.iter().any(|a| a.key == "total" && a.value == "3"));
        assert!(res.attributes.iter().any(|a| a.key == "verified" && a.value == "1"));
        assert!(res.attributes.iter().any(|a| a.key == "failed" && a.value == "2"));
    ...
    fn supersede_seal_below_min_validators_fails() {
        let mut deps = mock_deps_with_wasm();
        setup_contract(deps.as_mut());
        let (old_seal_id, _) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
    ...
    fn extend_expiration_works() {
        ...
        let res = execute(deps.as_mut(), mock_env(), requester_info, msg).unwrap();
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "extend_expiration"));
    ...
    fn supersede_seal_works() {
        let mut deps = mock_deps_with_wasm();
        setup_contract(deps.as_mut());
        let (old_seal_id, requester_info) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        ...
        let msg = ExecuteMsg::SupersedeSeal {
            ...
            validator_addresses: vec![VALIDATOR1.to_string(), VALIDATOR2.to_string(), VALIDATOR3.to_string()],
        };
        
        let res = execute(deps.as_mut(), mock_env(), requester_info, msg).unwrap();
        
        let new_seal_id = res.attributes.iter()
    ...
    fn batch_verify_works() {
        ...
        let (seal_id1, _) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        let (seal_id2, _) = create_seal(deps.as_mut(), "requester2", vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        ...
        let res = execute(deps.as_mut(), mock_env(), mock_info("anyone", &[]), msg).unwrap();
        assert!(res.attributes.iter().any(|a| a.key == "action" && a.value == "batch_verify"));
        assert!(res.attributes.iter().any(|a| a.key == "total" && a.value == "2"));
    }
    ...
    fn query_seal_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::Seal { seal_id: seal_id.clone() }).unwrap();
    ...
    fn query_list_seals_works() {
        ...
        create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        create_seal(deps.as_mut(), "requester2", vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        
        let res = query(deps.as_ref(), mock_env(), QueryMsg::ListSeals {
            status: None,
    ...
    fn query_verify_active_seal() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::Verify { seal_id: seal_id.clone() }).unwrap();
    ...
    fn query_job_history_works() {
        ...
        let res = query(deps.as_ref(), mock_env(), QueryMsg::JobSealHistory { job_id: "job123".to_string() }).unwrap();
        let seals: Vec<Seal> = from_json(&res).unwrap();
    ...
    fn seal_id_generation_unique() {
        ...
        let (seal_id1, _) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
        let (seal_id2, _) = create_seal(deps.as_mut(), REQUESTER, vec![VALIDATOR1, VALIDATOR2, VALIDATOR3]);
