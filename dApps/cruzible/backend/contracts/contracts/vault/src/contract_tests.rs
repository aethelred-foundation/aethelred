...
mod security_tests {
    ...
    use cosmwasm_std::{coins, from_json, Addr, CosmosMsg, BankMsg, coin, Env, MessageInfo, Uint128, OwnedDeps, Response};

    // ============ TEST HELPERS ============

    fn proper_instantiate() -> (OwnedDeps<MockStorage, MockApi, MockQuerier>, Env, MessageInfo) {
    ...
    fn stake(deps: &mut OwnedDeps<MockStorage, MockApi, MockQuerier>, env: &Env, sender: &str, amount: u128) -> Response {
        let info = mock_info(sender, &coins(amount, "aeth"));
        let msg = ExecuteMsg::Stake { validator: "validator1".to_string() };
        execute(deps.as_mut(), env.clone(), info, msg).unwrap()
    }

    fn unstake(deps: &mut OwnedDeps<MockStorage, MockApi, MockQuerier>, env: &Env, sender: &str, amount: u128) -> Response {
        let info = mock_info(sender, &[]);
        let msg = ExecuteMsg::Unstake { amount: Uint128::from(amount) };
        execute(deps.as_mut(), env.clone(), info, msg).unwrap()
    }
    ...
    fn test_attack_4_donation_does_not_inflate_shares() {
        ...
        let _state: State = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::State {}).unwrap()).unwrap();
        let _initial_rate: ExchangeRateResponse = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::ExchangeRate {}).unwrap()).unwrap();

        // User stakes
        let _ = stake(&mut deps, &env, "user1", 10_000_000);

        // Record share price
        let rate_before: ExchangeRateResponse = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::ExchangeRate {}).unwrap()).unwrap();

        // Simulate donation (admin sweeps it)
        let info = mock_info("creator", &[]);
        let msg = ExecuteMsg::SweepDonations { recipient: "treasury".to_string() };
        let _ = execute(deps.as_mut(), env.clone(), info, msg);

        // Share price should not change from donation
        let rate_after: ExchangeRateResponse = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::ExchangeRate {}).unwrap()).unwrap();
    ...
    fn test_attack_5_rounding_favors_protocol() {
        ...
        // Get shares minted
        let shares_attr = res.attributes.iter().find(|a| a.key == "shares_minted").unwrap();
        ...
        let user_stake: UserStake = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::UserStake { address: "user".to_string() }).unwrap()).unwrap();
        
        // Unstake the same amount
        let info = mock_info("user", &[]);
        let msg = ExecuteMsg::Unstake { amount: Uint128::from(1_000_001u128) };
        let res = execute(deps.as_mut(), env.clone(), info, msg).unwrap();
        
        let burned_attr = res.attributes.iter().find(|a| a.key == "shares_burned").unwrap();
    ...
    fn test_attack_7_zero_share_mint_blocked() {
        ...
        if let Ok(response) = res {
            let shares_attr = response.attributes.iter().find(|a| a.key == "shares_minted").unwrap();
            let shares: Uint128 = shares_attr.value.parse().unwrap();
    ...
    fn test_attack_18_queue_dos_blocked() {
        ...
        for i in 0..MAX_UNBONDING_REQUESTS + 5 {
            let info = mock_info("user", &[]);
            let msg = ExecuteMsg::Unstake { amount: Uint128::from(1_000_000u128) };
    ...
    fn test_slash_replay_protection() {
        ...
        let msg = ExecuteMsg::RecordSlash { slash_id: 1, amount: Uint128::from(10_000_000u128) };
    ...
    fn test_slash_affects_exchange_rate() {
        ...
        // Record slash
        let info = mock_info("operator", &[]);
        let msg = ExecuteMsg::RecordSlash { slash_id: 1, amount: Uint128::from(50_000_000u128) };
        execute(deps.as_mut(), env.clone(), info, msg).unwrap();
    ...
    fn test_invariant_solvency() {
        ...
        // Check solvency
        let solvency: bool = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::CheckSolvency {}).unwrap()).unwrap();
    ...
    fn test_invariant_share_conservation() {
        ...
        // Get initial state
        let state_before: State = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::State {}).unwrap()).unwrap();
        ...
        // Get state after
        let state_after: State = from_json(&query(deps.as_ref(), env.clone(), QueryMsg::State {}).unwrap()).unwrap();
    ...
    fn test_only_whitelisted_validator_allowed() {
        ...
        let info = mock_info("user", &coins(10_000_000, "aeth"));
        let msg = ExecuteMsg::Stake { validator: "evil_validator".to_string() };
        let err = execute(deps.as_mut(), env, info, msg).unwrap_err();
    ...
    fn test_operator_can_update_validators() {
        ...
        let info = mock_info("operator", &[]);
        let msg = ExecuteMsg::UpdateValidators { validators: vec!["validator2".to_string()] };
        assert!(execute(deps.as_mut(), env.clone(), info, msg).is_ok());
