...
export function useStablecoinConfig(symbol: string): StablecoinOnChainConfig {
  ...
  const result = data as readonly [
    boolean, boolean, number, string, string, string, string,
    bigint, bigint, number, number, number, number,
  ] | undefined;
...
export function useStablecoinAllowance(symbol: string) {
  ...
  const tokenKey = symbol === 'USDC' ? 'usdcToken' : symbol === 'USDT' ? 'usdtToken' : '';
  const tokenAddr = (tokenKey ? CONTRACT_ADDRESSES[tokenKey as keyof typeof CONTRACT_ADDRESSES] : '') as Address;
...
export function useBridgeOut() {
  ...
  const bridgeOut = useCallback(
    async (
      ...
      const asset = STABLECOIN_ASSETS[symbol];
      if (!asset) {
        addNotification('error', 'Unknown Asset', `Stablecoin "${symbol}" is not registered.`);
        return undefined;
      }
      ...
      if (!bridgeAddr) {
        addNotification('error', 'Configuration Error', 'Stablecoin bridge contract address not configured');
        return undefined;
      }

      if (!address) {
        addNotification('error', 'Wallet Required', 'Please connect your wallet to bridge stablecoins.');
        return undefined;
      }
      ...
      if (!tokenAddr) {
        addNotification('error', 'Configuration Error', `${symbol} token address not configured`);
        return undefined;
      }
      ...
      try {
        ...
        addNotification('info', 'Approving', `Please approve ${symbol} spending in your wallet...`);
        ...
        addNotification('info', 'Bridging', `Please confirm the ${symbol} bridge-out transaction...`);
        ...
        if (receipt.status === 'reverted') {
          addNotification('error', 'Bridge Reverted', 'The bridge transaction was reverted on-chain.');
          return undefined;
        }
