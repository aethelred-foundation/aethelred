...
function generateValidators(): ValidatorInfo[] {
  const baseVotingPowers = [
    12_400_000, 10_800_000, 9_200_000, 8_500_000, 7_800_000, 7_100_000,
    6_400_000, 5_900_000, 5_300_000, 4_800_000, 4_200_000, 3_700_000, 3_200_000,
    2_800_000, 2_400_000, 2_000_000, 1_600_000, 1_300_000, 1_000_000, 800_000,
  ];
  ...
  return VALIDATOR_NAMES.map((name, i) => {
    ...
    const performanceScore = Number(
      (85 + seededRange(seed + 10, 0, 14)).toFixed(1),
    );
    const securityScore = Number(
      (88 + seededRange(seed + 11, 0, 12)).toFixed(1),
    );
    ...
    const stakeWeight = Number(
      ((votingPower / totalStakeForWeight) * 100).toFixed(1),
    );
    ...
    const radarData = [
      ...
      {
        metric: "Stake Weight",
        value: Math.min(Math.round(stakeWeight * 10), 100),
      },
    ...
    const delegatorBreakdown = [
      {
        name: `aeth1${seededInt(seed + 90, 1000, 9999)}...a1`,
        amount: seededInt(seed + 91, 500000, 2000000),
        color: DELEGATOR_COLORS[0],
      },
      {
        name: `aeth1${seededInt(seed + 92, 1000, 9999)}...b2`,
        amount: seededInt(seed + 93, 300000, 800000),
        color: DELEGATOR_COLORS[1],
      },
      {
        name: `aeth1${seededInt(seed + 94, 1000, 9999)}...c3`,
        amount: seededInt(seed + 95, 200000, 500000),
        color: DELEGATOR_COLORS[2],
      },
      {
        name: `aeth1${seededInt(seed + 96, 1000, 9999)}...d4`,
        amount: seededInt(seed + 97, 100000, 300000),
        color: DELEGATOR_COLORS[3],
      },
      {
        name: `aeth1${seededInt(seed + 98, 1000, 9999)}...e5`,
        amount: seededInt(seed + 99, 50000, 150000),
        color: DELEGATOR_COLORS[4],
      },
      {
        name: "Others",
        amount: votingPower - seededInt(seed + 100, 1200000, 3500000),
        color: DELEGATOR_COLORS[5],
      },
    ];
    ...
    return {
      rank: i + 1,
      name,
      address: `aeth1${Array.from(
        { length: 6 },
        (_, j) => "0123456789abcdef"[seededInt(seed + 140 + j, 0, 15)],
      ).join("")}...${Array.from(
        { length: 4 },
        (_, j) => "0123456789abcdef"[seededInt(seed + 150 + j, 0, 15)],
      ).join("")}`,
      status,
      votingPower,
      selfStake,
...
function generateStakeGrowth(): { day: string; totalStake: number }[] {
  return Array.from({ length: 90 }, (_, i) => {
    ...
    return {
      ...
      totalStake: Number(
        (75 + seededRange(i * 37, 0, 14.4) + i * 0.15).toFixed(1),
      ),
    };
  });
}

function generateVotingPowerCumulative(): {
  rank: number;
  name: string;
  cumulative: number;
}[] {
...
const SLASHING_EVENTS: SlashingEvent[] = [
  ...
  {
    validator: "Nodefleet",
    reason: "Invalid TEE attestation",
    amount: 8500,
    epoch: 218,
    date: "Feb 18, 2026",
  },
  ...
  {
    validator: "InfStones",
    reason: "Downtime during epoch transition",
    amount: 3100,
    epoch: 240,
    date: "Mar 4, 2026",
  },
...
function StatusBadge({ status }: { status: ValidatorStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset ${c.bg} ${c.text} ${c.ring}`}
    >
      <span
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 py-12 lg:py-16">
        <div className="mb-8">
          ...
          <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
            Explore the Aethelred validator set securing the network through
            Proof-of-Useful-Work consensus. TEE-verified computation, geographic
            diversity, and hardware-attested security.
          </p>
        </div>
          ...
          {[
            ...
            >
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span
                  className={`text-xs ${stat.trend ? "text-emerald-400" : "text-slate-500"}`}
                >
                  {stat.sub}
                </span>
              </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        {top3.map((v, i) => (
          ...
          >
            {/* Rank badge */}
            <div
              className={`absolute top-4 right-4 w-10 h-10 rounded-full bg-gradient-to-br ${rankColors[i]} flex items-center justify-center shadow-md`}
            >
...
function ValidatorTableSection({
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | ValidatorStatus>(
    "all",
  );
              ...
              <tbody className="divide-y divide-slate-700/30">
                {filteredValidators.map((v) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`text-sm font-bold tabular-nums ${v.rank <= 3 ? "text-red-400" : "text-slate-500"}`}
                      >
                        #{v.rank}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      <p className="text-xs text-slate-500 tabular-nums">
                        {((v.votingPower / TOTAL_STAKED) * 100).toFixed(1)}%
                      </p>
                    </td>
          ...
          {filteredValidators.map((v) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                <Sparkline
                  data={v.sparklineData}
                  color={v.performanceScore >= 95 ? "#10B981" : "#F59E0B"}
                  height={20}
                  width={60}
                />
                <span className="text-xs text-slate-500">
                  {formatNumber(v.delegators)} delegators
                </span>
              </div>
...
function ValidatorDetailDrawer({
  validator,
  isOpen,
  onClose,
  onDelegate,
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: "Voting Power",
              value: formatStake(v.votingPower),
              sub: `${((v.votingPower / TOTAL_STAKED) * 100).toFixed(2)}% of total`,
            },
            { label: "Commission", value: `${v.commission}%`, sub: "Fee rate" },
            { label: "Uptime", value: `${v.uptime}%`, sub: "Last 30 epochs" },
            {
              label: "Delegators",
              value: formatNumber(v.delegators),
              sub: "Total count",
            },
            {
              label: "Self-Stake",
              value: formatStake(v.selfStake),
              sub: `${((v.selfStake / v.votingPower) * 100).toFixed(1)}% ratio`,
            },
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={v.blocksPerEpoch}>
                ...
                <RechartsTooltip
                  ...
                  formatter={(val: number) => [`${val} blocks`, "Produced"]}
                />
                <Bar
                  dataKey="blocks"
                  fill={BRAND.red}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <RechartsTooltip
                    ...
                    formatter={(val: number) => [formatStake(val), "Staked"]}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
        ...
        <div>
          ...
          <div className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/30">
            ...
            <div className="flex items-center justify-between mt-2">
              <span className="text-xs text-slate-500">
                {formatNumber(v.aiJobsCompleted)} completed
              </span>
              <span className="text-xs text-slate-500">
                {formatNumber(v.aiJobsFailed)} failed
              </span>
            </div>
          </div>
        </div>
...
function DelegationModal({
  ...
  const estimatedMonthlyReward = validator
    ...
    : 0;
  const commissionFee = validator
    ? estimatedMonthlyReward * (validator.commission / 100)
    : 0;
      ...
      {!wallet.connected ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-slate-400 mb-4">
            Connect your wallet to delegate tokens
          </p>
          <button
            ...
            <div className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/30 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-white tabular-nums">
                  {formatNumber(amountNum, 2)} AETHEL
                </span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-white tabular-nums">
                  {formatNumber(netMonthlyReward, 2)} AETHEL
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-400">Commission Fee</span>
                <span className="text-sm text-slate-500 tabular-nums">
                  -{formatNumber(commissionFee, 2)} AETHEL
                </span>
              </div>
            </div>
          )}
...
function ComparisonModal({
  ...
  const selected = selectedRanks
    .map((rank) => VALIDATORS.find((v) => v.rank === rank))
    .filter(Boolean) as ValidatorInfo[];
  ...
  const scores = selected.map((v) => ({
    name: v.name,
    score:
      v.performanceScore * 0.3 +
      v.uptime * 0.25 +
      (100 - v.commission * 3) * 0.2 +
      v.apy * 5 * 0.15 +
      v.jobCompletionRate * 0.1,
  }));
      ...
      <div className="space-y-6">
        ...
        <div className="divide-y divide-slate-700/30">
          {[
            {
              label: "Voting Power",
              getValue: (v: ValidatorInfo) => formatStake(v.votingPower),
            },
            {
              label: "Commission",
              getValue: (v: ValidatorInfo) => `${v.commission}%`,
            },
            { label: "Uptime", getValue: (v: ValidatorInfo) => `${v.uptime}%` },
            { label: "APY", getValue: (v: ValidatorInfo) => `${v.apy}%` },
            {
              label: "Delegators",
              getValue: (v: ValidatorInfo) => formatNumber(v.delegators),
            },
            {
              label: "Self-Stake",
              getValue: (v: ValidatorInfo) => formatStake(v.selfStake),
            },
            {
              label: "Performance",
              getValue: (v: ValidatorInfo) => `${v.performanceScore}`,
            },
            {
              label: "Security Score",
              getValue: (v: ValidatorInfo) => `${v.securityScore}`,
            },
            {
              label: "Job Success Rate",
              getValue: (v: ValidatorInfo) => `${v.jobCompletionRate}%`,
            },
            {
              label: "TEE Attestation",
              getValue: (v: ValidatorInfo) =>
                v.teeAttestation ? "Verified" : "Unverified",
            },
            {
              label: "Avg Latency",
              getValue: (v: ValidatorInfo) => `${v.avgLatency}ms`,
            },
            {
              label: "Blocks Produced",
              getValue: (v: ValidatorInfo) => formatNumber(v.blocksProduced),
            },
          ].map((metric) => (
            <div
              key={metric.label}
            ...
            >
              ...
              {selected.map((v) => (
                <span
                  key={v.rank}
                  className="text-sm font-medium text-white text-center tabular-nums"
                >
                  {metric.getValue(v)}
                </span>
              ))}
        ...
        <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4">
          ...
          <p className="text-sm text-slate-300">
            Based on a balanced weighting of performance, uptime, commission,
            APY, and job completion rate,{" "}
            <span className="font-semibold text-white">{best.name}</span> offers
            the best overall balance for delegators seeking reliable returns
            with minimal risk.
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Cumulative voting power by validator rank (Lorenz curve)
          </p>
    ...
    <section className="mb-12">
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Staking yield over the last 30 epochs
          </p>
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Number of validators by commission rate
          </p>
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Network-wide staked AETHEL over 90 days (in millions)
          </p>
...
function PerformanceAnalyticsSection() {
  ...
  const blockProductionData = top10
    .map((v) => ({
  ...
  const avgTeeRate = Number(
    (
      top10.reduce((s, v) => s + (v.teeAttestation ? 99.8 : 95.0), 0) /
      top10.length
    ).toFixed(1),
  );
    ...
    <section className="mb-12">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Total blocks produced, top 10 validators
          </p>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            PoUW job success rate, top 10 validators
          </p>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <GlassCard className="p-6" hover={false}>
          ...
          <p className="text-xs text-slate-500 mb-4">
            Network p50/p95/p99 latency over 30 epochs
          </p>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <GlassCard className="p-6" hover={false}>
          <h3 className="font-semibold text-white mb-1">
            TEE Attestation Success
          </h3>
          <p className="text-xs text-slate-500 mb-6">
            Network-wide hardware attestation verification rate
          </p>
          <div className="flex flex-col items-center justify-center">
            <div className="relative">
              <ProgressRing
                percentage={avgTeeRate}
                size={160}
                strokeWidth={10}
                color="#10B981"
              />
            ...
            <div className="mt-6 grid grid-cols-3 gap-4 w-full">
              <div className="text-center">
                <p className="text-lg font-bold text-white tabular-nums">
                  {VALIDATORS.filter((v) => v.teeAttestation).length}
                </p>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className="text-sm font-semibold text-red-400 tabular-nums">
                    -{formatNumber(event.amount)} AETHEL
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="mt-4 p-3 bg-emerald-500/5 rounded-xl border border-emerald-500/10">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-emerald-400">
              90% of validators have zero slashing events. Network maintains
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            ...
            <div>
              ...
              <p className="text-lg text-slate-400 leading-relaxed mb-8">
                Secure the Aethelred network while earning rewards through
                Proof-of-Useful-Work. Run verifiable AI computation inside
                hardware-attested enclaves.
              </p>
              ...
              <div className="space-y-4">
                {[
                  {
                    icon: <Lock className="w-5 h-5" />,
                    title: "100K AETHEL Minimum Stake",
                    desc: "Bond the required minimum to register as a validator candidate",
                  },
                  {
                    icon: <Cpu className="w-5 h-5" />,
                    title: "TEE Hardware Required",
                    desc: "Intel SGX/TDX or AMD SEV capable processors",
                  },
                  {
                    icon: <Activity className="w-5 h-5" />,
                    title: "99.5% Uptime SLA",
                    desc: "Maintain consistent uptime or face slashing penalties",
                  },
                ].map((req, i) => (
                ...
                {[
                  {
                    icon: <Cpu className="w-5 h-5" />,
                    label: "CPU",
                    value: "16+ cores, TEE-enabled",
                    sub: "Intel Xeon w/ SGX or AMD EPYC w/ SEV",
                  },
                  {
                    icon: <HardDrive className="w-5 h-5" />,
                    label: "Memory",
                    value: "128 GB RAM minimum",
                    sub: "ECC recommended for production",
                  },
                  {
                    icon: <Server className="w-5 h-5" />,
                    label: "GPU",
                    value: "NVIDIA A100 / H100",
                    sub: "40GB+ VRAM for AI workloads",
                  },
                  {
                    icon: <Globe className="w-5 h-5" />,
                    label: "Network",
                    value: "1 Gbps symmetric",
                    sub: "Low-latency, < 50ms to peers",
                  },
                ].map((spec, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="p-2 rounded-lg bg-red-500/10 text-red-400 flex-shrink-0">
