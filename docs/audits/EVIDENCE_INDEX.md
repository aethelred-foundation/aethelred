...
| Runtime governance lock regression | `go test ./x/pouw/keeper -run 'Test(UpdateParams|CB7_UpdateParams|ParamChangeProposal|Mainnet)'` and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper` | COLLECTED | The PoUW mainnet lock registry is now enforced by the live `UpdateParams` handler, so locked governance fields fail closed at runtime unless a separately attestable elevated-governance execution path exists |
