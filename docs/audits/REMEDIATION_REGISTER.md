...
| HS-2026-04-16-45 | Medium | Verify / Startup Reachability | Apply the shared endpoint safety validator to startup reachability probes in `x/verify/readiness.go` so malformed, metadata, or private-address verifier targets are classified as unreachable instead of being probed directly during node startup | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./x/verify/...` and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper ./x/vault/keeper` |
| HS-2026-04-16-46 | Medium | Verify / Remote EZKL Boundary | Apply the shared endpoint safety validator to the EZKL remote prover and remote verifier request paths so configured `/prove` and `/verify` targets fail closed on malformed, metadata, or private-address endpoints instead of issuing unchecked outbound requests | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./x/verify/ezkl` and `go test ./x/verify/...` |
| HS-2026-04-16-47 | Medium | Verify / DCAP Collateral Boundary | Apply the shared endpoint safety validator to DCAP collateral and CRL fetch paths so PCCS-derived URLs and CRL distribution-point fallbacks fail closed on malformed, metadata, or private-address endpoints instead of issuing unchecked outbound requests from the chain-side or mirrored worker verifier | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./x/verify/tee/...`, `go test ./services/tee-worker/l1-verifier/...`, and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper ./x/vault/keeper` |
| HS-2026-04-16-48 | Medium | Worker / Nitro Remote Boundary | Apply the shared endpoint safety validator to the mirrored worker Nitro remote executor and attestation-verifier request paths, and bound remote error-body reads, so configured executor/verifier endpoints fail closed on malformed, metadata, or private-address targets instead of issuing unchecked outbound requests or reading unbounded remote error payloads | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./services/tee-worker/l1-verifier/...` and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper ./x/vault/keeper` |
...
| HS-2026-04-16-55 | Medium | Worker / Execution API Boundary | Enforce loopback-or-bearer access on both TEE worker entrypoints, including the Docker-deployed `cmd/aethelred-tee-worker`, require an explicit API token for non-loopback exposure, and teach the remote Go TEE client to present that bearer token when configured instead of relying on network placement alone to protect `/health`, `/capabilities`, `/execute`, and `/verify` | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./services/tee-worker/executor`, `go test ./cmd/aethelred-tee-worker`, `go test ./app`, and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper ./x/vault/keeper` |
| HS-2026-04-16-56 | Medium | Worker / SGX TCB Truthfulness | Fail closed in the SGX TCB evaluator when the TCB-info evaluation backend is unavailable, instead of returning a placeholder `UpToDate` assessment that could be mistaken for a real platform state evaluation | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `cargo test --manifest-path services/tee-worker/nitro-sdk/Cargo.toml --features attestation-evidence sgx` and `cargo check --manifest-path services/tee-worker/nitro-sdk/Cargo.toml --features attestation-evidence` |
| HS-2026-04-16-57 | Medium | Nitro / Remote Worker Auth Alignment | Align the remaining Nitro remote executor and remote attestation-verifier clients with the worker’s loopback-or-bearer access model so chain-side and mirrored verifier requests attach the configured bearer token instead of silently drifting away from the hardened worker boundary | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./x/verify/tee`, `go test ./services/tee-worker/l1-verifier/...`, and `go test ./app ./x/pouw/keeper ./x/verify/... ./x/seal/keeper ./x/validator/keeper ./x/vault/keeper ./services/tee-worker/l1-verifier/...` |
| HS-2026-04-16-58 | Medium | PoUW / Threat-Model Truthfulness | Align `AS-16` and `AS-17` in the formal threat model and closeout flow with the live downtime-slashing and app-layer vote-extension-signing controls so audit open-item reporting tracks only genuine residual risk instead of stale “not implemented” narratives | `ramesh/protocol-hardening-sweep-20260416` / PR `#141` | `go test ./x/pouw/keeper -run 'TestThreatModel_RuntimeEnforcementNarratives|TestRunComprehensiveRetest_(HasOpenItems|OpenItemsReflectCurrentThreatModel)'` and `go test ./x/pouw/keeper` |
