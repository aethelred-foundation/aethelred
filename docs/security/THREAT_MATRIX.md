...
| INFRA-02 | **Supply Chain Attack** | Compromised dependency or build tooling injects malicious code. | H | M | Controlled dependency review with manual approval; SLSA Level 3 build provenance; reproducible builds; dependency pinning with hash verification; `go.sum` and `Cargo.lock` committed. | Security Team | `scripts/validate-repo-auditability.py` |
