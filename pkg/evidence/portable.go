...
func VerifyPortable(pe *PortableEvidence) error {
	...
	if len(pe.Bundle.ChainOfCustody) > 0 {
		if err := VerifyCustodyChain(pe.Bundle.ChainOfCustody); err != nil {
			return fmt.Errorf("evidence/portable: custody chain verification failed: %w", err)
		}
	}

	// Verify attestation trust anchors if provided.
	if len(pe.TrustAnchors) > 0 && len(pe.Bundle.Attestations) > 0 {
		for _, att := range pe.Bundle.Attestations {
			trusted := false
			for _, anchor := range pe.TrustAnchors {
...
func (pe *PortableEvidence) computePackageHash() (string, error) {
	type hashable struct {
		...
		TrustAnchors     []PlatformTrustAnchor `json:"trust_anchors"`
