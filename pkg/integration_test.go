...
// =========================================================================
// Test 0: Enterprise Trust Chain for Regulated Autonomy
// =========================================================================

func TestEnterpriseTrustChainPassportReceiptSealEvidenceExport(t *testing.T) {
	t.Parallel()
	...
	if err := agent.VerifyIdentity(identity); err != nil {
		t.Fatalf("verifying enterprise identity: %v", err)
	}

	passport, err := evidence.NewAgentPassportEvidence(identity)
	if err != nil {
		t.Fatalf("converting agent passport evidence: %v", err)
	}

	engine := policy.NewPolicyEngine(policy.DefaultEngineConfig())
	policySet := &policy.PolicySet{
		ID:       "treasury-release-allow",
		Name:     "Treasury Release Allow",
		Priority: 10,
		Enabled:  true,
		Rules: []policy.Rule{
	...
	_, receipt, err := engine.EvaluateAndSign(ctx, req, signerKey, "did:aethelred:policy-gateway-1", "")
	if err != nil {
		t.Fatalf("evaluating and signing policy decision: %v", err)
	}
	if err := policy.VerifySignedPolicyReceipt(receipt, &signerKey.PublicKey); err != nil {
		t.Fatalf("verifying signed policy receipt: %v", err)
	}

	receiptEvidence, err := evidence.NewPolicyReceiptEvidence(receipt)
	if err != nil {
		t.Fatalf("converting policy receipt evidence: %v", err)
	}

	modelHash := sha256.Sum256([]byte("treasury-model-v1"))
	inputHash := sha256.Sum256([]byte("treasury-input-001"))
	outputHash := sha256.Sum256([]byte("approved-payment-output-001"))
	...
	if err != nil {
		t.Fatalf("creating digital seal: %v", err)
	}

	sealEvidence := evidence.Seal{
		SealID:         sealResp.SealID,
		JobID:          "job-treasury-001",
		OutputHash:     hex.EncodeToString(outputHash[:]),
		ValidatorCount: len(sealResp.ValidatorSet),
		BlockHeight:    sealResp.BlockHeight,
		Timestamp:      sealResp.Timestamp.UTC().Format(time.RFC3339Nano),
	}
	traceLink, err := evidence.NewTraceLink(identity, receipt, sealEvidence, "enterprise trust chain")
	if err != nil {
		t.Fatalf("creating evidence trace link: %v", err)
	}

	bundle := evidence.NewEvidenceBundle("Trust Kernel")
	bundle.AddRecord(evidence.Record{
		ID:        "record-enterprise-trust-chain-001",
		Type:      "governance",
		...
		Data: map[string]string{
			...
			"seal_id":           sealResp.SealID,
			"workflow":          "treasury_release",
		},
	})
	bundle.AddAgentPassport(passport)
	bundle.AddPolicyReceipt(receiptEvidence)
	bundle.AddSeal(sealEvidence)
	bundle.AddTraceLink(traceLink)

	if err := bundle.Finalize(""); err != nil {
		t.Fatalf("finalizing evidence bundle: %v", err)
	}
	if err := bundle.Validate(); err != nil {
		t.Fatalf("validating evidence bundle: %v", err)
	}

	port, err := evidence.Package(bundle, true)
	if err != nil {
		t.Fatalf("packaging evidence bundle: %v", err)
	}
	if err := evidence.VerifyPortable(port); err != nil {
		t.Fatalf("verifying portable evidence package: %v", err)
	}

	store := sdk.NewInMemoryEvidenceStore()
	policyAttachment, err := sdk.NewPolicyReceiptAttachment(receipt)
	if err != nil {
		t.Fatalf("creating policy receipt attachment: %v", err)
	}
	passportAttachment, err := sdk.NewAgentPassportAttachment(identity)
	if err != nil {
		t.Fatalf("creating agent passport attachment: %v", err)
	}
	traceAttachment, err := sdk.NewTraceLinkAttachment(sdk.TraceLink{
		ID:                traceLink.ID,
		AgentDID:          traceLink.AgentDID,
		PolicyReceiptID:   traceLink.PolicyReceiptID,
		PolicyReceiptHash: traceLink.PolicyReceiptHash,
		SealID:            traceLink.SealID,
		OutputHash:        traceLink.OutputHash,
		LinkedAt:          traceLink.LinkedAt,
		Description:       traceLink.Description,
	})
	if err != nil {
		t.Fatalf("creating trace link attachment: %v", err)
	}
	if err := sdk.AttachEvidence(store, sealResp.SealID, policyAttachment); err != nil {
		t.Fatalf("attaching policy receipt evidence: %v", err)
	}
	if err := sdk.AttachEvidence(store, sealResp.SealID, passportAttachment); err != nil {
		t.Fatalf("attaching passport evidence: %v", err)
	}
	if err := sdk.AttachEvidence(store, sealResp.SealID, traceAttachment); err != nil {
		t.Fatalf("attaching trace link evidence: %v", err)
	}

	sealBundle, err := sdk.CreateBundle(store, &sealtypes.DigitalSeal{
		Id:               sealResp.SealID,
		ModelCommitment:  modelHash[:],
		InputCommitment:  inputHash[:],
		OutputCommitment: outputHash[:],
		RequestedBy:      sealResp.RequestedBy,
		Purpose:          sealResp.Purpose,
		Status:           sealResp.Status,
	})
	if err != nil {
		t.Fatalf("creating seal evidence bundle: %v", err)
	}
	if err := sdk.VerifyBundle(sealBundle); err != nil {
		t.Fatalf("verifying seal evidence bundle: %v", err)
	}
}

func TestEnterpriseControlLedgerAuditorExports(t *testing.T) {
	t.Parallel()
	...
	engine := policy.NewPolicyEngine(policy.DefaultEngineConfig())
	if err := engine.RegisterPolicySet(&policy.PolicySet{
		ID:       "ledger-export-policy",
		Name:     "Ledger Export Policy",
		Priority: 10,
		Enabled:  true,
		Rules: []policy.Rule{
			policy.NewAllowRule("allow_release", []policy.Condition{{Field: "risk_tier", Operator: policy.Equals, Value: "low"}}),
		},
	}); err != nil {
		t.Fatalf("registering policy set: %v", err)
	}
	...
	_, receipt, err := engine.EvaluateAndSign(ctx, &policy.EvaluationRequest{
		Actor:    identity.AgentID(),
		Action:   "payments.release",
		Resource: "acct:treasury-main",
		Context: map[string]string{
	...
	}, signerKey, "did:aethelred:policy-gateway-1", "")
	if err != nil {
	...
	passport, err := evidence.NewAgentPassportEvidence(identity)
	if err != nil {
		t.Fatalf("converting passport: %v", err)
	}
	receiptEvidence, err := evidence.NewPolicyReceiptEvidence(receipt)
	if err != nil {
		t.Fatalf("converting receipt: %v", err)
	}
	outputHash := sha256.Sum256([]byte("ledger-export-output"))
	seal := evidence.Seal{
		SealID:         "seal-ledger-export-001",
		JobID:          "job-ledger-export-001",
		OutputHash:     hex.EncodeToString(outputHash[:]),
		ValidatorCount: 4,
		BlockHeight:    512,
		Timestamp:      time.Now().UTC().Format(time.RFC3339Nano),
	}
	traceLink, err := evidence.NewTraceLink(identity, receipt, seal, "auditor export chain")
	if err != nil {
		t.Fatalf("creating trace link: %v", err)
	}
	trustDoc := auditexport.BuildPouwTrustComplianceExport(
		time.Now().UTC().Format(time.RFC3339Nano),
		&pouwkeeper.QueryModuleStatusResponse{BlockHeight: 512, CurrentEpoch: 9, TotalUWU: 144},
		&pouwkeeper.EnterpriseAuditTrustRegistryStatus{Configured: true, Version: "2026.04.14", Source: "pouw_governance"},
		&pouwkeeper.EnterpriseAuditTrustRegistry{Version: "2026.04.14", Source: "pouw_governance"},
		[]pouwkeeper.AuditRecord{{
			Sequence:   11,
			Action:     "enterprise_audit_trust_registry_updated",
			Actor:      "gov-authority",
			Timestamp:  time.Now().UTC().Format(time.RFC3339Nano),
			RecordHash: evidence.EvidenceHashHex([]byte("governance-history")),
		}},
		&pouwkeeper.ComplianceReport{
			...
			Controls: []pouwkeeper.ComplianceControl{{
				...
				Artifact:     "pkg/evidence/control_ledger.go",
				EvidenceType: "code",
				Status:       pouwkeeper.ComplianceStatusMapped,
			}},
		},
	)
	trustPayload, err := auditexport.ExportPouwTrustComplianceJSON(trustDoc)
	if err != nil {
		t.Fatalf("exporting trust compliance payload: %v", err)
	}
	trustPackage, err := auditexport.CreatePouwTrustCompliancePackage(trustDoc, "json", trustPayload)
	if err != nil {
		t.Fatalf("creating trust compliance package: %v", err)
	}
	trustPackage.AuditAnchor = &pouwkeeper.AuditRecord{
		Sequence:     12,
		PreviousHash: "governance-history",
		...
		Details: map[string]string{
			"package_hash":  trustPackage.Manifest.PackageHash,
			"payload_hash":  trustPackage.Manifest.PayloadHash,
			"document_hash": trustPackage.Manifest.DocumentHash,
		},
	}
	trustPackage.AuditAnchor.RecordHash = auditpkg.ComputeRecordHash(*trustPackage.AuditAnchor)
	trustArtifact, err := auditexport.ToEvidenceTrustCompliancePackage(trustPackage)
	if err != nil {
		t.Fatalf("converting trust compliance package into evidence artifact: %v", err)
	}

	ledger := evidence.NewControlLedger("Trust Kernel")
	ledger.WithMetadata("workflow", "treasury_release")
	ledger.AddRecord(evidence.Record{
		ID:        "record-ledger-export-001",
		Type:      "governance",
		Action:    "payments.release.authorized",
		Actor:     identity.AgentID(),
		Timestamp: time.Now().UTC().Format(time.RFC3339Nano),
	...
	if err := ledger.AddTrustCompliancePackage(trustArtifact); err != nil {
		t.Fatalf("adding trust compliance package to ledger: %v", err)
	}
	if err := ledger.AddControl(evidence.LedgerControl{
		ControlID:   "CTRL-LEDGER-001",
		ControlName: "Treasury Release Approval",
		Status:      evidence.ControlSatisfied,
		EvidenceRefs: evidence.ControlEvidenceRefs{
			RecordIDs:                 []string{"record-ledger-export-001"},
			PolicyReceiptIDs:          []string{receipt.ID},
			SealIDs:                   []string{seal.SealID},
			TraceLinkIDs:              []string{traceLink.ID},
			TrustCompliancePackageIDs: []string{trustArtifact.ID},
		},
	}); err != nil {
		t.Fatalf("adding ledger control: %v", err)
	}
	if err := ledger.Finalize(""); err != nil {
		t.Fatalf("finalizing ledger: %v", err)
	}
	if err := ledger.Validate(); err != nil {
		t.Fatalf("validating ledger: %v", err)
	}

	jsonBytes, err := evidenceexport.ExportControlLedgerJSON(ledger)
	if err != nil {
		t.Fatalf("exporting control ledger json: %v", err)
	}
	if len(jsonBytes) == 0 {
		t.Fatal("expected non-empty JSON export")
	}

	csvBytes, err := evidenceexport.ExportControlLedgerCSV(ledger)
	if err != nil {
		t.Fatalf("exporting control ledger csv: %v", err)
	}
	if len(csvBytes) == 0 {
		t.Fatal("expected non-empty CSV export")
	}

	oscalBytes, err := evidenceexport.ExportControlLedgerOSCAL(ledger)
	if err != nil {
		t.Fatalf("exporting control ledger oscal: %v", err)
	}
	if len(oscalBytes) == 0 {
		t.Fatal("expected non-empty OSCAL export")
	}

	trustAttachment, err := sdk.NewTrustCompliancePackageAttachment(trustPackage)
	if err != nil {
		t.Fatalf("creating trust compliance seal attachment: %v", err)
	}
	if err := sdk.AttachEvidence(sdk.NewInMemoryEvidenceStore(), seal.SealID, trustAttachment); err != nil {
		t.Fatalf("attaching trust compliance seal evidence: %v", err)
	}
}
