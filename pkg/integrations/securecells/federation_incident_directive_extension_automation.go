...
const (
	secureCellFederationIncidentDirectiveExtensionReviewSLA     = 6 * time.Hour
	secureCellFederationIncidentDirectiveExtensionResolutionSLA = 24 * time.Hour
)

type secureCellFederationIncidentDirectiveExtensionOverdueState struct {
...
type secureCellFederationIncidentDirectiveExtensionAutomationPlan struct {
	action           string
	trigger          string
	reason           string
	pendingAction    string
...
// ListOverdueFederationIncidentDirectiveExtensions returns operator-facing
// projections for exception reviews or dispute resolutions whose governed
// deadline has elapsed.
func (s *Service) ListOverdueFederationIncidentDirectiveExtensions(_ context.Context, filter SecureCellOverdueFederationIncidentDirectiveExtensionFilter) ([]SecureCellOverdueFederationIncidentDirectiveExtension, error) {
	if s == nil {
		return nil, nil
	}
	s.mu.RLock()
	...
	items := make([]SecureCellOverdueFederationIncidentDirectiveExtension, 0)
		...
		for _, response := range run.result.FederationIncidentResponses {
			...
			for _, directive := range response.IncidentDirectives {
				...
				for _, extension := range directive.Extensions {
					...
					if filter.Status != "" && extension.Status != filter.Status {
						continue
					}
					overdue, ok := secureCellFederationIncidentDirectiveExtensionOverdueStateForAt(extension, at)
					if !ok {
						continue
					}
...
// ListFederationIncidentDirectiveExtensionAutomationActions returns automated
// exception-governance actions already applied by the runtime.
func (s *Service) ListFederationIncidentDirectiveExtensionAutomationActions(_ context.Context, filter SecureCellFederationIncidentDirectiveExtensionAutomationActionFilter) ([]SecureCellFederationIncidentDirectiveExtensionAutomationActionRecord, error) {
	if s == nil {
		return nil, nil
	}
	s.mu.RLock()
	...
	items := make([]SecureCellFederationIncidentDirectiveExtensionAutomationActionRecord, 0)
	for _, run := range s.runs {
		if run == nil || run.result == nil {
			continue
		}
...
// SweepFederationIncidentDirectiveExtensions applies automated escalation or
// fail-closed contract suspension to overdue directive exception reviews.
func (s *Service) SweepFederationIncidentDirectiveExtensions(ctx context.Context, at time.Time, lifecycle SecureCellLifecycleRequest) (*SecureCellFederationIncidentDirectiveExtensionSweepResult, error) {
	if s == nil {
	...
	report := &SecureCellFederationIncidentDirectiveExtensionSweepResult{
		At:           at.UTC(),
	...
	for _, cellID := range cellIDs {
		...
		for _, response := range run.result.FederationIncidentResponses {
			for _, directive := range response.IncidentDirectives {
				report.ExtensionsScanned += len(directive.Extensions)
				for _, extension := range directive.Extensions {
					plan, ok := secureCellFederationIncidentDirectiveExtensionAutomationPlanFromRun(run, response, directive, extension, at)
					if !ok {
						continue
					}
					...
					baseMetadata := mergeStringMaps(lifecycle.Metadata, map[string]string{
						...
						"federation_incident_directive_extension_action":         plan.action,
						"federation_incident_directive_extension_trigger":        plan.trigger,
						...
						"federation_incident_directive_extension_pending_action": plan.pendingAction,
						"federation_incident_directive_extension_due_at":         plan.dueAt.UTC().Format(time.RFC3339Nano),
...
func secureCellFederationIncidentDirectiveExtensionOverdueStateForAt(extension SecureCellFederationIncidentDirectiveExtension, at time.Time) (secureCellFederationIncidentDirectiveExtensionOverdueState, bool) {
	switch extension.Status {
	case SecureCellFederationIncidentDirectiveExtensionStatusPendingReview:
		if extension.CreatedAt.IsZero() {
			return secureCellFederationIncidentDirectiveExtensionOverdueState{}, false
		}
		reviewDueAt := extension.CreatedAt.UTC().Add(secureCellFederationIncidentDirectiveExtensionReviewSLA)
		if reviewDueAt.After(at) {
			return secureCellFederationIncidentDirectiveExtensionOverdueState{}, false
		}
		return secureCellFederationIncidentDirectiveExtensionOverdueState{
			pendingAction:    "review",
			automationAction: "escalate_response",
			overdueReason:    "directive extension review deadline reached",
	...
	case SecureCellFederationIncidentDirectiveExtensionStatusDisputed:
		dispute := secureCellPendingFederationIncidentDirectiveExtensionDispute(extension)
		if dispute == nil || dispute.CreatedAt.IsZero() {
			return secureCellFederationIncidentDirectiveExtensionOverdueState{}, false
		}
		resolutionDueAt := dispute.CreatedAt.UTC().Add(secureCellFederationIncidentDirectiveExtensionResolutionSLA)
		if resolutionDueAt.After(at) {
			return secureCellFederationIncidentDirectiveExtensionOverdueState{}, false
		}
		return secureCellFederationIncidentDirectiveExtensionOverdueState{
			pendingAction:    "resolve_dispute",
			automationAction: "suspend_contracts",
			overdueReason:    "directive extension dispute resolution deadline reached",
...
func secureCellFederationIncidentDirectiveExtensionAutomationPlanFromRun(run *secureCellRun, response SecureCellFederationIncidentResponse, _ SecureCellFederationIncidentDirective, extension SecureCellFederationIncidentDirectiveExtension, at time.Time) (secureCellFederationIncidentDirectiveExtensionAutomationPlan, bool) {
	if run == nil || run.result == nil {
...
func secureCellFederationIncidentDirectiveExtensionAutomationAlreadyApplied(run *secureCellRun, extensionID string, extensionStatus SecureCellFederationIncidentDirectiveExtensionStatus, pendingAction string, action string, targetDID string) bool {
	...
	for _, transition := range run.result.Transitions {
		...
		if !strings.EqualFold(strings.TrimSpace(transition.Metadata["federation_incident_directive_extension_id"]), extensionID) {
			continue
		}
		if !strings.EqualFold(strings.TrimSpace(transition.Metadata["federation_incident_directive_extension_pending_action"]), pendingAction) {
			continue
		}
