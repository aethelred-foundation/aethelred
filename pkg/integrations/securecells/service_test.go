...
func TestService_FederationIncidentDirectiveExtensionAppealReconciliationChallengeAutomation(t *testing.T) {
	...
	if _, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeal(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealRulingRequest{
		ActorDID:          participantA.AgentID(),
		BoardParty:        SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:            SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify,
		RulingSummary:     "First local board vote ratified the challenge appeal ruling",
		RulingDescription: "The first local board reviewer ratified the reconciliation challenge appeal ruling, but the threshold is not met yet.",
		EvidenceIDs:       []string{directiveID, comparisonKey, challengeAppealID},
		Reason:            "record first reconciliation challenge appeal board vote",
		Metadata:          map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-VOTE-01"},
	}); err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeal first vote failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals after first vote failed: %v", err)
	}
	if len(challengeAppeals) != 1 || challengeAppeals[0].BoardRecordedVoteCount != 1 || challengeAppeals[0].BoardMissingQuorumCount != 1 || challengeAppeals[0].RatifyVoteCount != 1 {
		t.Fatalf("expected under-quorum reconciliation challenge appeal after first vote, got %+v", challengeAppeals)
	}
	challengeAppealReviewOverdueAt := challengeAppeals[0].CreatedAt.UTC().Add(secureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBoardReviewSLA + time.Hour)

	overdueChallengeAppeals, err := service.ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals(ctx, SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealFilter{
		CellID:            created.CellID,
		AppealID:          appealID,
		ComparisonKey:     comparisonKey,
		ChallengeID:       challengeID,
		ChallengeAppealID: challengeAppealID,
		Before:            &challengeAppealReviewOverdueAt,
	})
	if err != nil {
		t.Fatalf("ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals review failed: %v", err)
	}
	if len(overdueChallengeAppeals) != 1 || overdueChallengeAppeals[0].PendingAction != "board_review" || overdueChallengeAppeals[0].AutomationAction != "delegate_review_committee" || overdueChallengeAppeals[0].BoardDelegationCount != 0 || overdueChallengeAppeals[0].BoardMissingQuorumCount != 1 {
		t.Fatalf("expected overdue reconciliation challenge appeal board-review posture, got %+v", overdueChallengeAppeals)
	}

	challengeAppealSweepActor := "did:aethelred:appeal-reconciliation-challenge-appeal-sweeper"
	challengeAppealSweep, err := service.SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals(ctx, challengeAppealReviewOverdueAt, SecureCellLifecycleRequest{
		ActorDID: challengeAppealSweepActor,
		Reason:   "automated reconciliation challenge appeal board-review sweep",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-SWEEP-01"},
	})
	if err != nil {
		t.Fatalf("SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals review failed: %v", err)
	}
	if challengeAppealSweep.CellsMutated == 0 || challengeAppealSweep.AppealsScanned == 0 {
		t.Fatalf("expected reconciliation challenge appeal review sweep to mutate state, got %+v", challengeAppealSweep)
	}

	challengeAppealAutomationActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAutomationActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAutomationActionFilter{
		CellID:            created.CellID,
		AppealID:          appealID,
		ComparisonKey:     comparisonKey,
		ChallengeID:       challengeID,
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAutomationActions review failed: %v", err)
	}
	if len(challengeAppealAutomationActions) != 1 || challengeAppealAutomationActions[0].PendingAction != "board_review" || challengeAppealAutomationActions[0].AutomatedActor != challengeAppealSweepActor || challengeAppealAutomationActions[0].Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_review_delegated" || challengeAppealAutomationActions[0].TargetDID == "" || challengeAppealAutomationActions[0].BoardDelegationCount != 1 {
		t.Fatalf("expected one automated reconciliation challenge appeal review action, got %+v", challengeAppealAutomationActions)
	}
	delegatedChallengeAppealReviewer := challengeAppealAutomationActions[0].TargetDID

	reconciliationBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationBundle(ctx, created.CellID, comparisonKey, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationBundle after challenge appeal review sweep failed: %v", err)
	}
	if len(reconciliationBundle.ChallengeAppealAutomationActions) != 1 || reconciliationBundle.ChallengeAppealAutomationActions[0].PendingAction != "board_review" || reconciliationBundle.ChallengeAppealAutomationActions[0].Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_review_delegated" {
		t.Fatalf("expected reconciliation bundle to include challenge appeal automation trail, got %+v", reconciliationBundle.ChallengeAppealAutomationActions)
	}
	foundChallengeAppealAutomationControl := false
	for _, control := range reconciliationBundle.Controls {
		if control.ControlID == "CELL-FED-25" {
			foundChallengeAppealAutomationControl = true
			break
		}
	}
	if !foundChallengeAppealAutomationControl {
		t.Fatalf("expected reconciliation bundle controls to include CELL-FED-25, got %+v", reconciliationBundle.Controls)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after challenge appeal review sweep failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions) != 1 || casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions[0].PendingAction != "board_review" || casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions[0].Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_review_delegated" {
		t.Fatalf("expected case pack to include challenge appeal automation trail, got %+v", casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions)
	}
	...
	ruledChallengeAppeal, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeal(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealRulingRequest{
		ActorDID:          delegatedChallengeAppealReviewer,
		BoardParty:        SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:            SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify,
	...
	if len(challengeAppeals) != 1 || challengeAppeals[0].ChallengeAppealID != challengeAppealID || challengeAppeals[0].ChallengeAppealStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealStatusRatified || challengeAppeals[0].BoardDelegationCount != 1 || challengeAppeals[0].RatifyVoteCount != 2 {
		t.Fatalf("expected one ratified reconciliation challenge appeal after ruling, got %+v", challengeAppeals)
	}
	if challengeAppeals[0].RuledAt == nil || challengeAppeals[0].RuledAt.IsZero() {
		t.Fatalf("expected ruled reconciliation challenge appeal timestamp, got %+v", challengeAppeals)
	}

	challengeAppealAckOverdueAt := challengeAppeals[0].RuledAt.UTC().Add(secureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAcknowledgementSLA + time.Hour)
	overdueChallengeAppeals, err = service.ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals(ctx, SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealFilter{
		CellID:            created.CellID,
		AppealID:          appealID,
		ComparisonKey:     comparisonKey,
		ChallengeID:       challengeID,
		ChallengeAppealID: challengeAppealID,
		Before:            &challengeAppealAckOverdueAt,
	})
	if err != nil {
		t.Fatalf("ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals acknowledgement failed: %v", err)
	}
	if len(overdueChallengeAppeals) != 1 || overdueChallengeAppeals[0].PendingAction != "acknowledge_enforcement" || (overdueChallengeAppeals[0].AutomationAction != "escalate_response" && overdueChallengeAppeals[0].AutomationAction != "suspend_contracts") {
		t.Fatalf("expected overdue reconciliation challenge appeal acknowledgement posture, got %+v", overdueChallengeAppeals)
	}

	challengeAppealSweep, err = service.SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals(ctx, challengeAppealAckOverdueAt, SecureCellLifecycleRequest{
		ActorDID: challengeAppealSweepActor,
		Reason:   "automated reconciliation challenge appeal acknowledgement sweep",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-SWEEP-02"},
	})
	if err != nil {
		t.Fatalf("SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppeals acknowledgement failed: %v", err)
	}
	if challengeAppealSweep.CellsMutated == 0 || challengeAppealSweep.AppealsScanned == 0 {
		t.Fatalf("expected reconciliation challenge appeal acknowledgement sweep to mutate state, got %+v", challengeAppealSweep)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAutomationActions acknowledgement failed: %v", err)
	}
	if len(challengeAppealAutomationActions) != 2 {
		t.Fatalf("expected two automated reconciliation challenge appeal actions, got %+v", challengeAppealAutomationActions)
	}
	foundReviewAutomation := false
	foundAckAutomation := false
	for _, item := range challengeAppealAutomationActions {
		if item.AutomatedActor != challengeAppealSweepActor {
			t.Fatalf("expected automated actor %s, got %+v", challengeAppealSweepActor, challengeAppealAutomationActions)
		}
		...
		case "board_review":
			if item.Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_review_delegated" {
				t.Fatalf("expected board-review automation to preserve delegated review action, got %+v", challengeAppealAutomationActions)
			}
			foundReviewAutomation = true
		case "acknowledge_enforcement":
			if item.Action != "secure_cell.federation_incident_response_escalated" && item.Action != "secure_cell.federation_contract_suspended" {
				t.Fatalf("expected acknowledgement automation action to escalate or suspend, got %+v", challengeAppealAutomationActions)
			}
			foundAckAutomation = true
		}
	}
	if !foundReviewAutomation || !foundAckAutomation {
		t.Fatalf("expected challenge appeal automation to cover review and acknowledgement, got %+v", challengeAppealAutomationActions)
	}

	reconciliationBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationBundle(ctx, created.CellID, comparisonKey, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationBundle after challenge appeal acknowledgement sweep failed: %v", err)
	}
	if len(reconciliationBundle.ChallengeAppealAutomationActions) != 2 {
		t.Fatalf("expected reconciliation bundle to include both challenge appeal automation actions, got %+v", reconciliationBundle.ChallengeAppealAutomationActions)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after challenge appeal acknowledgement sweep failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions) != 2 {
		t.Fatalf("expected case pack to include both challenge appeal automation actions, got %+v", casePack.DirectiveExtensionAppealReconciliationChallengeAppealAutomationActions)
	}
	...
	if len(challengeAppeals) != 1 || challengeAppeals[0].ChallengeAppealID != challengeAppealID || challengeAppeals[0].EnforcementAcknowledgedBy != participantB.AgentID() || challengeAppeals[0].RatifyVoteCount != 2 {
	...
	if len(challengeAppealActions) != 5 || challengeAppealActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealActionEnforcementAcknowledged || challengeAppealActions[4].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealActionAppealed {
	...
	acknowledgedChallengeAppealAlignment, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAcknowledgeRequest{
		ActorDID: owner.AgentID(),
		Reason:   "acknowledge aligned reciprocal challenge appeal bundle",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment failed: %v", err)
	}
	if !controlLedgerHasControl(acknowledgedChallengeAppealAlignment.ControlLedger, "CELL-FED-29") {
		t.Fatalf("expected acknowledged challenge appeal alignment control ledger to include CELL-FED-29, got %+v", acknowledgedChallengeAppealAlignment.ControlLedger.Controls)
	}

	tamperedChallengeAppealBundle := *challengeAppealBundle
	tamperedChallengeAppealBundle.ChallengeAppealSummary.Ruling = SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn
	disputedChallengeAppealAlignment, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   &tamperedChallengeAppealBundle,
		Reason:   "ingest divergent reciprocal challenge appeal bundle for governed dispute",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-INGEST-01"},
	})
	if err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle tampered failed: %v", err)
	}
	disputedChallengeAppealAlignment, err = service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentDisputeRequest{
		ActorDID:    owner.AgentID(),
		Reason:      "dispute latest reciprocal challenge appeal bundle alignment",
		Divergences: []string{"counterparty challenge appeal ruling diverged after bundle tampering"},
		Metadata:    map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment failed: %v", err)
	}
	if !controlLedgerHasControl(disputedChallengeAppealAlignment.ControlLedger, "CELL-FED-29") {
		t.Fatalf("expected disputed challenge appeal alignment control ledger to include CELL-FED-29, got %+v", disputedChallengeAppealAlignment.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppeals after dispute failed: %v", err)
	}
	if len(counterpartyChallengeAppeals) != 2 || counterpartyChallengeAppeals[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealStatusInvalid || counterpartyChallengeAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentReviewStatusDisputed || counterpartyChallengeAppeals[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentReviewStatusAcknowledged {
		t.Fatalf("expected latest invalid disputed reciprocal challenge appeal plus earlier acknowledged snapshot, got %+v", counterpartyChallengeAppeals)
	}
	alignmentActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActionFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActions failed: %v", err)
	}
	if len(alignmentActions) != 2 || alignmentActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActionDispute || alignmentActions[0].SnapshotID == "" || alignmentActions[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentReviewStatusDisputed || alignmentActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentActionAcknowledge || alignmentActions[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentReviewStatusAcknowledged {
		t.Fatalf("expected reciprocal challenge appeal alignment action trail, got %+v", alignmentActions)
	}
	...
	if len(reconciliationBundle.ChallengeAppeals) != 1 || len(reconciliationBundle.ChallengeAppealActions) != 5 || len(reconciliationBundle.ChallengeAppealAutomationActions) != 2 || reconciliationBundle.ChallengeAppeals[0].ChallengeAppealID != challengeAppealID {
		t.Fatalf("expected reconciliation bundle to include challenge appeal artifacts, got %+v", reconciliationBundle)
	}
	challengeAppealBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle after alignment governance failed: %v", err)
	}
	if len(challengeAppealBundle.CounterpartyAlignmentActions) != 2 {
		t.Fatalf("expected challenge appeal bundle to include reciprocal alignment actions, got %+v", challengeAppealBundle)
	}
	...
	acknowledgedAlignmentResponse, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomation(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomationAcknowledgeRequest{
		ActorDID:              owner.AgentID(),
		CounterpartyReference: "counterparty-alignment-automation-ack-0001",
		Reason:                "acknowledge automated reciprocal challenge appeal alignment supervision",
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomation failed: %v", err)
	}
	if !controlLedgerHasControl(acknowledgedAlignmentResponse.ControlLedger, "CELL-FED-31") {
		t.Fatalf("expected acknowledged alignment response control ledger to include CELL-FED-31, got %+v", acknowledgedAlignmentResponse.ControlLedger.Controls)
	}

	correctedAlignmentResponse, err := service.AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrection(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrectionAttestationRequest{
		ActorDID:               owner.AgentID(),
		CounterpartySnapshotID: counterpartyChallengeAppeals[0].SnapshotID,
		CounterpartyReference:  "counterparty-alignment-automation-correct-0001",
		Reason:                 "attest corrected handling for automated reciprocal challenge appeal alignment supervision",
		Metadata:               map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-CORRECT-01"},
	})
	if err != nil {
		t.Fatalf("AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrection failed: %v", err)
	}
	if !controlLedgerHasControl(correctedAlignmentResponse.ControlLedger, "CELL-FED-31") {
		t.Fatalf("expected corrected alignment response control ledger to include CELL-FED-31, got %+v", correctedAlignmentResponse.ControlLedger.Controls)
	}

	resolvedAlignmentResponse, err := service.AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResolution(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResolutionAttestationRequest{
		ActorDID:              owner.AgentID(),
		CounterpartyReference: "counterparty-alignment-automation-resolve-0001",
		Reason:                "resolve automated reciprocal challenge appeal alignment supervision",
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-RESOLVE-01"},
	})
	if err != nil {
		t.Fatalf("AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResolution failed: %v", err)
	}
	if !controlLedgerHasControl(resolvedAlignmentResponse.ControlLedger, "CELL-FED-31") {
		t.Fatalf("expected resolved alignment response control ledger to include CELL-FED-31, got %+v", resolvedAlignmentResponse.ControlLedger.Controls)
	}

	alignmentResponseActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActions failed: %v", err)
	}
	if len(alignmentResponseActions) != 3 ||
		alignmentResponseActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionResolve ||
		alignmentResponseActions[0].ResponseStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusResolved ||
		alignmentResponseActions[0].AlignmentAutomationTransitionID != alignmentAutomationActions[0].TransitionID ||
		alignmentResponseActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionCorrect ||
		alignmentResponseActions[2].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionAcknowledge {
		t.Fatalf("expected resolved reciprocal challenge appeal alignment response trail, got %+v", alignmentResponseActions)
	}

	alignmentResponseBundle, err := service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle failed: %v", err)
	}
	if err := VerifyFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(alignmentResponseBundle); err != nil {
		t.Fatalf("VerifyFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle failed: %v", err)
	}
	if alignmentResponseBundle.ResponseStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusResolved ||
		alignmentResponseBundle.ResponseActionCount != 3 ||
		len(alignmentResponseBundle.ResponseActions) != 3 ||
		alignmentResponseBundle.ChallengeAppealBundleHash == "" {
		t.Fatalf("expected resolved alignment response bundle projection, got %+v", alignmentResponseBundle)
	}

	challengeAppealBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle after alignment response failed: %v", err)
	}
	if len(challengeAppealBundle.CounterpartyAlignmentResponseActions) != 3 {
		t.Fatalf("expected challenge appeal bundle to include reciprocal alignment response actions, got %+v", challengeAppealBundle)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after alignment response failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActions) != 3 {
		t.Fatalf("expected case pack to include reciprocal alignment response actions, got %+v", casePack)
	}

	tamperedAlignmentResponseBundle := *challengeAppealBundle
	tamperedAlignmentResponseBundle.ChallengeAppealSummary.Ruling = SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn
	if _, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   &tamperedAlignmentResponseBundle,
		Reason:   "ingest divergent reciprocal challenge appeal bundle for automated response dispute",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-DISPUTE-INGEST-01"},
	}); err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle for automated response dispute failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppeals after automated response dispute ingest failed: %v", err)
	}
	if len(counterpartyChallengeAppeals) < 1 || counterpartyChallengeAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentReviewStatusUnreviewed || counterpartyChallengeAppeals[0].AlignmentStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentStatusDivergent {
		t.Fatalf("expected latest divergent unreviewed reciprocal challenge appeal snapshot, got %+v", counterpartyChallengeAppeals)
	}

	disputeOverdueAt := counterpartyChallengeAppeals[0].ReceivedAt.Add(3 * time.Hour)
	if _, err := service.SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignments(ctx, disputeOverdueAt, SecureCellLifecycleRequest{
		ActorDID: challengeAppealSweepActor,
		Reason:   "automated reciprocal challenge appeal alignment dispute sweep",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-DISPUTE-SWEEP-01"},
	}); err != nil {
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomationActions after dispute sweep failed: %v", err)
	}
	if len(alignmentAutomationActions) < 2 || alignmentAutomationActions[0].PendingAction != "dispute_alignment" {
		t.Fatalf("expected latest reciprocal challenge appeal alignment automation action to require dispute, got %+v", alignmentAutomationActions)
	}

	disputedAlignmentResponse, err := service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomation(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomationDisputeRequest{
		ActorDID:              owner.AgentID(),
		CounterpartyReference: "counterparty-alignment-automation-dispute-0001",
		Reason:                "dispute automated reciprocal challenge appeal alignment supervision",
		Divergences:           []string{"counterparty challenge appeal ruling diverged after automated alignment supervision"},
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-AUTO-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAutomation failed: %v", err)
	}
	if !controlLedgerHasControl(disputedAlignmentResponse.ControlLedger, "CELL-FED-31") {
		t.Fatalf("expected disputed alignment response control ledger to include CELL-FED-31, got %+v", disputedAlignmentResponse.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActions after dispute failed: %v", err)
	}
	if len(alignmentResponseActions) != 4 ||
		alignmentResponseActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActionDispute ||
		alignmentResponseActions[0].ResponseStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusDisputed ||
		alignmentResponseActions[0].AlignmentAutomationPendingAction != "dispute_alignment" {
		t.Fatalf("expected latest disputed reciprocal challenge appeal alignment response action, got %+v", alignmentResponseActions)
	}

	alignmentResponseBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle after dispute failed: %v", err)
	}
	if alignmentResponseBundle.ResponseStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusDisputed || alignmentResponseBundle.ResponseActionCount != 4 {
		t.Fatalf("expected disputed alignment response bundle projection, got %+v", alignmentResponseBundle)
	}

	challengeAppealBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle after automated response dispute failed: %v", err)
	}
	if len(challengeAppealBundle.CounterpartyAlignmentResponseActions) != 4 {
		t.Fatalf("expected challenge appeal bundle to include disputed reciprocal alignment response actions, got %+v", challengeAppealBundle)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after automated response dispute failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseActions) != 4 {
		t.Fatalf("expected case pack to include disputed reciprocal alignment response actions, got %+v", casePack)
	}

	currentAfterAlignmentResponse, err := service.GetCell(ctx, created.CellID)
	if err != nil {
		t.Fatalf("GetCell after alignment response governance failed: %v", err)
	}
	if !controlLedgerHasControl(currentAfterAlignmentResponse.ControlLedger, "CELL-FED-31") {
		t.Fatalf("expected current control ledger to include CELL-FED-31 after alignment response governance, got %+v", currentAfterAlignmentResponse.ControlLedger.Controls)
	}
	...
	if !controlLedgerHasControl(ruledChallengeAppeal.ControlLedger, "CELL-FED-27") {
		t.Fatalf("expected ruled challenge appeal control ledger to include CELL-FED-27, got %+v", ruledChallengeAppeal.ControlLedger.Controls)
	}
	...
	appealedAlignmentResponse, err := service.AppealFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponse(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRequest{
		ActorDID:                            owner.AgentID(),
		...
		Summary:                             "Local org appealed disputed alignment response",
		Description:                         "The local organization escalated the disputed alignment response into governed correction-board review.",
		EvidenceIDs:                         []string{directiveID, comparisonKey, challengeAppealID},
		CorrectionBoardReviewThreshold:      2,
		EligibleCorrectionBoardReviewerDIDs: []string{participantA.AgentID()},
		Reason:                              "appeal disputed alignment response",
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-01"},
	})
	if err != nil {
		t.Fatalf("AppealFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponse failed: %v", err)
	}

	alignmentResponseAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals failed: %v", err)
	}
	if len(alignmentResponseAppeals) != 1 || alignmentResponseAppeals[0].Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusPendingCorrectionBoardReview || alignmentResponseAppeals[0].BoardReviewThreshold != 2 {
		t.Fatalf("expected one pending alignment response appeal, got %+v", alignmentResponseAppeals)
	}
	responseAppealID := alignmentResponseAppeals[0].ResponseAppealID

	delegatedAlignmentResponseAppeal, err := service.DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionDelegationRequest{
		ActorDID:  owner.AgentID(),
		TargetDID: owner.AgentID(),
		Reason:    "delegate correction-board quorum restoration",
		Metadata:  map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-DELEGATE-01"},
	})
	if err != nil {
		t.Fatalf("DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview failed: %v", err)
	}

	firstAlignmentResponseAppealRuling, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             participantA.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn,
		RulingSummary:        "First correction-board vote overturned the disputed response",
		RulingDescription:    "The first correction-board reviewer favored overturning the disputed alignment response, but the threshold is not satisfied yet.",
		EvidenceIDs:          []string{directiveID, comparisonKey, responseAppealID},
		Reason:               "record first correction-board appeal vote",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-VOTE-01"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal first vote failed: %v", err)
	}

	ruledAlignmentResponseAppeal, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             owner.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn,
		RulingSummary:        "Correction-board overturned disputed response",
		RulingDescription:    "The delegated reviewer completed the correction-board quorum and overturned the disputed alignment response.",
		EvidenceIDs:          []string{directiveID, comparisonKey, responseAppealID},
		Reason:               "complete correction-board appeal ruling",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-VOTE-02"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal final vote failed: %v", err)
	}

	acknowledgedAlignmentResponseAppeal, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealEnforcement(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAcknowledgeRequest{
		ActorDID:           participantB.AgentID(),
		AcknowledgingParty: SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		Summary:            "Counterparty acknowledged correction-board ruling",
		Description:        "The counterparty accepted enforcement of the overturned alignment response after the correction-board ruling.",
		EvidenceIDs:        []string{directiveID, comparisonKey, responseAppealID},
		Reason:             "acknowledge correction-board alignment response appeal enforcement",
		Metadata:           map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealEnforcement failed: %v", err)
	}

	alignmentResponseAppeals, err = service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		Status:            SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusEnforcementAcknowledged,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals final failed: %v", err)
	}
	if len(alignmentResponseAppeals) != 1 || alignmentResponseAppeals[0].ResponseAppealID != responseAppealID || alignmentResponseAppeals[0].OverturnVoteCount != 2 || alignmentResponseAppeals[0].EnforcementAcknowledgedBy != participantB.AgentID() {
		t.Fatalf("expected one acknowledged alignment response appeal, got %+v", alignmentResponseAppeals)
	}

	alignmentResponseAppealActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  responseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActions failed: %v", err)
	}
	if len(alignmentResponseAppealActions) != 5 ||
		alignmentResponseAppealActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionEnforcementAcknowledged ||
		alignmentResponseAppealActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionRuled ||
		alignmentResponseAppealActions[2].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionVoteRecorded ||
		alignmentResponseAppealActions[3].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionReviewDelegated ||
		alignmentResponseAppealActions[4].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActionAppealed {
		t.Fatalf("expected full alignment response appeal action trail, got %+v", alignmentResponseAppealActions)
	}

	alignmentResponseBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle after appeal failed: %v", err)
	}
	if alignmentResponseBundle.ResponseAppealCount != 1 ||
		alignmentResponseBundle.ResponseAppealStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusEnforcementAcknowledged ||
		len(alignmentResponseBundle.ResponseAppeals) != 1 ||
		len(alignmentResponseBundle.ResponseAppealActions) != 5 {
		t.Fatalf("expected alignment response bundle to include correction-board appeal governance, got %+v", alignmentResponseBundle)
	}

	challengeAppealBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle after alignment response appeal failed: %v", err)
	}
	if len(challengeAppealBundle.CounterpartyAlignmentResponseAppeals) != 1 || len(challengeAppealBundle.CounterpartyAlignmentResponseAppealActions) != 5 {
		t.Fatalf("expected challenge appeal bundle to include alignment response appeal trails, got %+v", challengeAppealBundle)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after alignment response appeal failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals) != 1 || len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealActions) != 5 {
		t.Fatalf("expected case pack to include alignment response appeal governance, got %+v", casePack)
	}

	ingestedAlignmentResponseBundle, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   alignmentResponseBundle,
		Reason:   "ingest exact reciprocal correction-board ruling bundle",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-BUNDLE-INTAKE-01"},
	})
	if err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle failed: %v", err)
	}
	if !controlLedgerHasControl(ingestedAlignmentResponseBundle.ControlLedger, "CELL-FED-34") {
		t.Fatalf("expected ingested alignment response bundle control ledger to include CELL-FED-34, got %+v", ingestedAlignmentResponseBundle.ControlLedger.Controls)
	}

	counterpartyAlignmentResponseAppeals, err := service.ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseFilter{
		CellID:            created.CellID,
		OrganizationID:    organizationID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  responseAppealID,
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals failed: %v", err)
	}
	if len(counterpartyAlignmentResponseAppeals) != 1 || counterpartyAlignmentResponseAppeals[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusVerified || counterpartyAlignmentResponseAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusUnreviewed || counterpartyAlignmentResponseAppeals[0].MatchedLocalResponseAppealID != responseAppealID {
		t.Fatalf("expected one verified unreviewed reciprocal alignment response appeal snapshot, got %+v", counterpartyAlignmentResponseAppeals)
	}

	acknowledgedCounterpartyRuling, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyRuling(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyAcknowledgeRequest{
		ActorDID:                     owner.AgentID(),
		...
		CounterpartyReference:        "counterparty-alignment-response-appeal-ruling-ack-0001",
		Reason:                       "acknowledge exact reciprocal correction-board ruling",
		Metadata:                     map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-RULING-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyRuling failed: %v", err)
	}
	if !controlLedgerHasControl(acknowledgedCounterpartyRuling.ControlLedger, "CELL-FED-35") {
		t.Fatalf("expected acknowledged counterparty ruling control ledger to include CELL-FED-35, got %+v", acknowledgedCounterpartyRuling.ControlLedger.Controls)
	}

	tamperedAlignmentResponseAppealBundle := *alignmentResponseBundle
	tamperedAlignmentResponseAppealBundle.ResponseAppeals = append([]SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealSummary(nil), alignmentResponseBundle.ResponseAppeals...)
	tamperedAlignmentResponseAppealBundle.ResponseAppeals[0].Ruling = SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify
	if _, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   &tamperedAlignmentResponseAppealBundle,
		Reason:   "ingest divergent reciprocal correction-board ruling bundle",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-BUNDLE-INTAKE-02"},
	}); err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle tampered failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after tamper failed: %v", err)
	}
	if len(counterpartyAlignmentResponseAppeals) != 2 || counterpartyAlignmentResponseAppeals[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseStatusInvalid || counterpartyAlignmentResponseAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusUnreviewed || counterpartyAlignmentResponseAppeals[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusAcknowledged {
		t.Fatalf("expected latest invalid unreviewed reciprocal alignment response appeal snapshot plus earlier acknowledged snapshot, got %+v", counterpartyAlignmentResponseAppeals)
	}

	disputedCounterpartyRuling, err := service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyRuling(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyDisputeRequest{
		ActorDID:                     owner.AgentID(),
		CounterpartySnapshotID:       counterpartyAlignmentResponseAppeals[0].SnapshotID,
		CounterpartyResponseAppealID: counterpartyAlignmentResponseAppeals[0].ResponseAppealID,
		CounterpartyReference:        "counterparty-alignment-response-appeal-ruling-dispute-0001",
		Reason:                       "dispute exact reciprocal correction-board ruling",
		Divergences:                  []string{"counterparty correction-board ruling diverged after bundle tampering"},
		Metadata:                     map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-RULING-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyRuling failed: %v", err)
	}
	if !controlLedgerHasControl(disputedCounterpartyRuling.ControlLedger, "CELL-FED-35") {
		t.Fatalf("expected disputed counterparty ruling control ledger to include CELL-FED-35, got %+v", disputedCounterpartyRuling.ControlLedger.Controls)
	}

	counterpartyResponseAppealActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  responseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActions failed: %v", err)
	}
	if len(counterpartyResponseAppealActions) != 2 ||
		counterpartyResponseAppealActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionDispute ||
		counterpartyResponseAppealActions[0].CounterpartySnapshotID != counterpartyAlignmentResponseAppeals[0].SnapshotID ||
		counterpartyResponseAppealActions[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusDisputed ||
		counterpartyResponseAppealActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionAcknowledge ||
		counterpartyResponseAppealActions[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusAcknowledged {
		t.Fatalf("expected exact reciprocal correction-board ruling review trail, got %+v", counterpartyResponseAppealActions)
	}

	escalatedCounterpartyDispute, err := service.EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyDispute(ctx, created.CellID, responseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyDisputeEscalationRequest{
		ActorDID:                            owner.AgentID(),
		CounterpartySnapshotID:              counterpartyAlignmentResponseAppeals[0].SnapshotID,
		CounterpartyResponseAppealID:        counterpartyAlignmentResponseAppeals[0].ResponseAppealID,
		CounterpartyReference:               "counterparty-alignment-response-appeal-ruling-dispute-0001",
		...
		EvidenceIDs:                         []string{challengeAppealID, responseAppealID},
		CorrectionBoardReviewThreshold:      1,
		EligibleCorrectionBoardReviewerDIDs: []string{participantA.AgentID()},
		Reason:                              "escalate disputed counterparty correction-board ruling into rehearing",
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-RULING-ESCALATE-01"},
	})
	if err != nil {
		t.Fatalf("EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyDispute failed: %v", err)
	}
	if !controlLedgerHasControl(escalatedCounterpartyDispute.ControlLedger, "CELL-FED-37") {
		t.Fatalf("expected escalated counterparty ruling control ledger to include CELL-FED-37, got %+v", escalatedCounterpartyDispute.ControlLedger.Controls)
	}

	escalatedResponseAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:                 created.CellID,
		ChallengeAppealID:      challengeAppealID,
		ParentResponseAppealID: responseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after counterparty escalation failed: %v", err)
	}
	if len(escalatedResponseAppeals) != 1 ||
		escalatedResponseAppeals[0].ParentResponseAppealID != responseAppealID ||
		escalatedResponseAppeals[0].Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusPendingCorrectionBoardReview ||
		!strings.EqualFold(strings.TrimSpace(escalatedResponseAppeals[0].Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_counterparty_review_escalated"]), "true") ||
		!strings.EqualFold(strings.TrimSpace(escalatedResponseAppeals[0].Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_counterparty_review_snapshot_id"]), strings.TrimSpace(counterpartyAlignmentResponseAppeals[0].SnapshotID)) {
		t.Fatalf("expected one pending counterparty-escalated rehearing, got %+v", escalatedResponseAppeals)
	}

	escalatedAlignmentResponseBundle, err := service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle after counterparty escalation failed: %v", err)
	}
	foundCounterpartyEscalation := false
	for _, item := range escalatedAlignmentResponseBundle.ResponseAppeals {
		if item.ParentResponseAppealID == responseAppealID && strings.EqualFold(strings.TrimSpace(item.Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_counterparty_review_escalated"]), "true") {
			foundCounterpartyEscalation = true
			break
		}
	}
	if !foundCounterpartyEscalation {
		t.Fatalf("expected alignment response bundle to include counterparty-escalated rehearing lineage, got %+v", escalatedAlignmentResponseBundle.ResponseAppeals)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActions after escalation failed: %v", err)
	}
	foundEscalatedCounterpartyReview := false
	for _, item := range counterpartyResponseAppealActions {
		if item.Action == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionEscalate &&
			item.ReviewStatus == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusEscalated &&
			item.ResponseAppealID == responseAppealID &&
			item.LocalResponseAppealStatus == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusPendingCorrectionBoardReview {
			foundEscalatedCounterpartyReview = true
			break
		}
	}
	if len(counterpartyResponseAppealActions) != 2 || !foundEscalatedCounterpartyReview {
		t.Fatalf("expected counterparty escalation review trail to include escalated rehearing, got %+v", counterpartyResponseAppealActions)
	}
	...
	counterpartyAlignmentResponseAppeals, err = service.ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseFilter{
		...
		OrganizationID:    organizationID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  counterpartyAlignmentResponseAppeals[0].ResponseAppealID,
		SnapshotID:        counterpartyResponseAppealActions[0].CounterpartySnapshotID,
	})
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after escalation failed: %v", err)
	}
	if len(counterpartyAlignmentResponseAppeals) != 1 || counterpartyAlignmentResponseAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusEscalated {
		t.Fatalf("expected reciprocal correction-board ruling posture to move to escalated, got %+v", counterpartyAlignmentResponseAppeals)
	}

	resolvedCounterpartyDispute, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, escalatedResponseAppeals[0].ResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             participantA.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn,
		RulingSummary:        "Counterparty-dispute rehearing overturned local ruling",
		RulingDescription:    "The delegated reviewer completed the rehearing and overturned the prior local ruling after the counterparty dispute was escalated.",
		EvidenceIDs:          []string{escalatedResponseAppeals[0].ResponseAppealID},
		Reason:               "close counterparty-dispute rehearing after escalation verification",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-RULING-ESCALATE-RULE-01"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal after counterparty escalation failed: %v", err)
	}
	if !controlLedgerHasControl(resolvedCounterpartyDispute.ControlLedger, "CELL-FED-38") {
		t.Fatalf("expected resolved counterparty ruling control ledger to include CELL-FED-38, got %+v", resolvedCounterpartyDispute.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActions after resolution failed: %v", err)
	}
	foundResolvedCounterpartyReview := false
	for _, item := range counterpartyResponseAppealActions {
		if item.Action == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyActionResolve &&
			item.ReviewStatus == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusResolved &&
			item.ResponseAppealID == responseAppealID &&
			item.LocalRuling == SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn &&
			item.LocalResponseAppealStatus == SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed {
			foundResolvedCounterpartyReview = true
			break
		}
	}
	if len(counterpartyResponseAppealActions) != 3 || !foundResolvedCounterpartyReview {
		t.Fatalf("expected counterparty resolution review trail to include ruled rehearing outcome, got %+v", counterpartyResponseAppealActions)
	}
	...
	counterpartyAlignmentResponseAppeals, err = service.ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseFilter{
		...
		OrganizationID:    organizationID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  counterpartyResponseAppealActions[0].CounterpartyResponseAppealID,
		SnapshotID:        counterpartyResponseAppealActions[0].CounterpartySnapshotID,
	})
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after resolution failed: %v", err)
	}
	if len(counterpartyAlignmentResponseAppeals) != 1 || counterpartyAlignmentResponseAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewStatusResolved || counterpartyAlignmentResponseAppeals[0].ReviewActionCount != 3 {
	...
	ingestedCounterpartyReviewAppealBundle, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   counterpartyReviewAppealBundle,
		Reason:   "ingest reciprocal imported-ruling rehearing board bundle",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-INTAKE-01"},
	})
	if err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle failed: %v", err)
	}
	if !controlLedgerHasControl(ingestedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-39") {
		t.Fatalf("expected ingested counterparty review appeal bundle control ledger to include CELL-FED-39, got %+v", ingestedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}

	counterpartyReviewAppealSnapshots, err := service.ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppeals(ctx, SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealFilter{
		CellID:            created.CellID,
		OrganizationID:    organizationID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  responseAppealID,
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppeals failed: %v", err)
	}
	if len(counterpartyReviewAppealSnapshots) != 1 ||
		counterpartyReviewAppealSnapshots[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealStatusVerified ||
		counterpartyReviewAppealSnapshots[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusUnreviewed ||
		strings.TrimSpace(counterpartyReviewAppealSnapshots[0].SnapshotID) == "" ||
		counterpartyReviewAppealSnapshots[0].ResponseAppealID != responseAppealID {
		t.Fatalf("expected one verified unreviewed reciprocal imported-ruling rehearing board snapshot, got %+v", counterpartyReviewAppealSnapshots)
	}

	storedCounterpartyReviewAppealBundle, err := service.GetFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle(ctx, created.CellID, counterpartyReviewAppealSnapshots[0].SnapshotID)
	if err != nil {
		t.Fatalf("GetFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle failed: %v", err)
	}
	if storedCounterpartyReviewAppealBundle == nil ||
		strings.TrimSpace(storedCounterpartyReviewAppealBundle.ID) != strings.TrimSpace(counterpartyReviewAppealBundle.ID) ||
		strings.TrimSpace(storedCounterpartyReviewAppealBundle.ContentHash) != strings.TrimSpace(counterpartyReviewAppealBundle.ContentHash) ||
		storedCounterpartyReviewAppealBundle.ReviewAppeal.AppealReviewID != counterpartyReviewAppealBundle.ReviewAppeal.AppealReviewID {
		t.Fatalf("expected stored reciprocal imported-ruling rehearing board bundle to match the original, got %+v", storedCounterpartyReviewAppealBundle)
	}

	acknowledgedCounterpartyReviewAppealBundle, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealAcknowledgeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            counterpartyReviewAppealSnapshots[0].SnapshotID,
		CounterpartyReference: "counterparty-review-appeal-ruling-ack-0001",
		Reason:                "acknowledge reciprocal imported-ruling rehearing board bundle",
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealRuling failed: %v", err)
	}
	if !controlLedgerHasControl(acknowledgedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-40") {
		t.Fatalf("expected acknowledged counterparty review appeal control ledger to include CELL-FED-40, got %+v", acknowledgedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}

	tamperedCounterpartyReviewAppealBundle := *counterpartyReviewAppealBundle
	tamperedCounterpartyReviewAppealBundle.ReviewAppeal.BoardRuling = SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify
	if _, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   &tamperedCounterpartyReviewAppealBundle,
		Reason:   "ingest divergent reciprocal imported-ruling rehearing board bundle",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-INTAKE-02"},
	}); err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealBundle tampered failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppeals after tamper failed: %v", err)
	}
	if len(counterpartyReviewAppealSnapshots) != 2 ||
		counterpartyReviewAppealSnapshots[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealStatusInvalid ||
		counterpartyReviewAppealSnapshots[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusUnreviewed ||
		counterpartyReviewAppealSnapshots[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusAcknowledged {
		t.Fatalf("expected latest invalid unreviewed reciprocal imported-ruling rehearing board snapshot plus earlier acknowledged snapshot, got %+v", counterpartyReviewAppealSnapshots)
	}

	disputedCounterpartyReviewAppealBundle, err := service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealDisputeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            counterpartyReviewAppealSnapshots[0].SnapshotID,
		CounterpartyReference: "counterparty-review-appeal-ruling-dispute-0001",
		Reason:                "dispute divergent reciprocal imported-ruling rehearing board bundle",
		Divergences:           []string{"counterparty imported-ruling rehearing board ruling diverged after bundle tampering"},
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealRuling failed: %v", err)
	}
	if !controlLedgerHasControl(disputedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-40") {
		t.Fatalf("expected disputed counterparty review appeal control ledger to include CELL-FED-40, got %+v", disputedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}

	counterpartyReviewAppealActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  responseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActions failed: %v", err)
	}
	if len(counterpartyReviewAppealActions) != 2 ||
		counterpartyReviewAppealActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionDispute ||
		counterpartyReviewAppealActions[0].SnapshotID != counterpartyReviewAppealSnapshots[0].SnapshotID ||
		counterpartyReviewAppealActions[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusDisputed ||
		counterpartyReviewAppealActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionAcknowledge ||
		counterpartyReviewAppealActions[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusAcknowledged {
		t.Fatalf("expected reciprocal imported-ruling rehearing board review trail with dispute then acknowledge, got %+v", counterpartyReviewAppealActions)
	}

	escalatedCounterpartyReviewAppealBundle, err := service.EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealDispute(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealDisputeEscalationRequest{
		ActorDID:                            owner.AgentID(),
		SnapshotID:                          counterpartyReviewAppealSnapshots[0].SnapshotID,
		CounterpartyReference:               "counterparty-review-appeal-ruling-escalate-0001",
		AppealingParty:                      SecureCellFederationIncidentResponsePartyLocalOrg,
		CorrectionBoardParty:                SecureCellFederationIncidentResponsePartyLocalOrg,
		EnforcementAcknowledgementParty:     SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		...
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-ESCALATE-01"},
	})
	if err != nil {
		t.Fatalf("EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealDispute failed: %v", err)
	}
	if !controlLedgerHasControl(escalatedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-40") {
		t.Fatalf("expected escalated counterparty review appeal control ledger to include CELL-FED-40, got %+v", escalatedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}

	currentRun, err := service.getRun(created.CellID)
	if err != nil {
		t.Fatalf("getRun after counterparty review appeal escalation failed: %v", err)
	}
	escalatedCounterpartyReviewBoardAppeal := secureCellLatestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealForChallengeAppeal(currentRun, challengeAppealID)
	if escalatedCounterpartyReviewBoardAppeal == nil ||
		!strings.EqualFold(strings.TrimSpace(escalatedCounterpartyReviewBoardAppeal.Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_counterparty_review_appeal_escalated"]), "true") ||
		!strings.EqualFold(strings.TrimSpace(escalatedCounterpartyReviewBoardAppeal.Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_counterparty_review_appeal_snapshot_id"]), strings.TrimSpace(counterpartyReviewAppealSnapshots[0].SnapshotID)) ||
		!strings.EqualFold(strings.TrimSpace(escalatedCounterpartyReviewBoardAppeal.ParentResponseAppealID), strings.TrimSpace(counterpartyReviewAppealSnapshots[0].CounterpartyBoardResponseAppealID)) {
		t.Fatalf("expected latest appeal to be the escalated reciprocal imported-ruling rehearing board appeal, got %+v", escalatedCounterpartyReviewBoardAppeal)
	}

	resolvedCounterpartyReviewAppealBundle, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, escalatedCounterpartyReviewBoardAppeal.ResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             owner.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingOverturn,
		RulingSummary:        "Resolve disputed imported reciprocal appeal-board ruling via governed rehearing",
		RulingDescription:    "The fresh correction-board rehearing overturned the prior imported reciprocal appeal-board ruling.",
		EvidenceIDs:          []string{"counterparty-review-appeal-resolution"},
		Reason:               "resolve disputed imported reciprocal appeal-board ruling via governed rehearing",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-RESOLVE-01"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal for escalated counterparty review appeal failed: %v", err)
	}
	if !controlLedgerHasControl(resolvedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-40") {
		t.Fatalf("expected resolved counterparty review appeal control ledger to include CELL-FED-40, got %+v", resolvedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}
	if !controlLedgerHasControl(resolvedCounterpartyReviewAppealBundle.ControlLedger, "CELL-FED-41") {
		t.Fatalf("expected resolved counterparty review appeal control ledger to include CELL-FED-41, got %+v", resolvedCounterpartyReviewAppealBundle.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppeals after escalation failed: %v", err)
	}
	if len(counterpartyReviewAppealSnapshots) != 1 ||
		counterpartyReviewAppealSnapshots[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusResolved ||
		counterpartyReviewAppealSnapshots[0].ReviewActionCount != 3 {
		t.Fatalf("expected disputed reciprocal imported-ruling rehearing board snapshot to move to resolved with three review actions, got %+v", counterpartyReviewAppealSnapshots)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActions after escalation failed: %v", err)
	}
	if len(counterpartyReviewAppealActions) != 3 ||
		counterpartyReviewAppealActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionResolve ||
		counterpartyReviewAppealActions[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusResolved ||
		counterpartyReviewAppealActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionEscalate ||
		counterpartyReviewAppealActions[1].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusEscalated ||
		counterpartyReviewAppealActions[2].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewActionDispute ||
		counterpartyReviewAppealActions[2].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusDisputed {
		t.Fatalf("expected reciprocal imported-ruling rehearing board review trail with dispute, escalation, and resolution, got %+v", counterpartyReviewAppealActions)
	}

	counterpartyReviewAppealReviewBundle, err := service.BuildFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundle(ctx, created.CellID, counterpartyReviewAppealSnapshots[0].SnapshotID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundle failed: %v", err)
	}
	if err := VerifyFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundle(counterpartyReviewAppealReviewBundle); err != nil {
		t.Fatalf("VerifyFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewBundle failed: %v", err)
	}
	if counterpartyReviewAppealReviewBundle.Review.ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusResolved ||
		len(counterpartyReviewAppealReviewBundle.ReviewActions) != 3 ||
		counterpartyReviewAppealReviewBundle.LocalBoardResponseAppeal == nil ||
		counterpartyReviewAppealReviewBundle.LocalBoardResponseAppeal.Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed ||
		len(counterpartyReviewAppealReviewBundle.LocalBoardResponseActions) == 0 ||
		strings.TrimSpace(counterpartyReviewAppealReviewBundle.CounterpartyReviewAppealBundleHash) == "" ||
		strings.TrimSpace(counterpartyReviewAppealReviewBundle.CounterpartyReviewBundleHash) == "" ||
		strings.TrimSpace(counterpartyReviewAppealReviewBundle.AlignmentResponseBundleHash) == "" ||
		strings.TrimSpace(counterpartyReviewAppealReviewBundle.ChallengeAppealBundleHash) == "" {
		t.Fatalf("expected signed reciprocal imported-ruling rehearing board review bundle with local appeal evidence, got %+v", counterpartyReviewAppealReviewBundle)
	}

	counterpartyReviewAppealReviewAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppeals failed: %v", err)
	}
	if len(counterpartyReviewAppealReviewAppeals) != 1 ||
		counterpartyReviewAppealReviewAppeals[0].ResponseAppealID != escalatedCounterpartyReviewBoardAppeal.ResponseAppealID ||
		counterpartyReviewAppealReviewAppeals[0].ResponseAppealStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed ||
		counterpartyReviewAppealReviewAppeals[0].ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusResolved ||
		counterpartyReviewAppealReviewAppeals[0].PriorBoardResponseAppealID != counterpartyReviewAppealSnapshots[0].CounterpartyBoardResponseAppealID ||
		counterpartyReviewAppealReviewAppeals[0].PriorBoardResponseAppealStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed {
		t.Fatalf("expected one resolved imported-ruling review rehearing board summary, got %+v", counterpartyReviewAppealReviewAppeals)
	}

	counterpartyReviewAppealReviewAppealBundle, err := service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundle(ctx, created.CellID, counterpartyReviewAppealReviewAppeals[0].ResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundle failed: %v", err)
	}
	if err := VerifyFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealBundle(counterpartyReviewAppealReviewAppealBundle); err != nil {
	...
	if counterpartyReviewAppealReviewAppealBundle.ReviewAppeal.ResponseAppealID != escalatedCounterpartyReviewBoardAppeal.ResponseAppealID ||
		counterpartyReviewAppealReviewAppealBundle.ReviewAppeal.ResponseAppealStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed ||
		counterpartyReviewAppealReviewAppealBundle.ReviewAppeal.ReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewStatusResolved ||
		counterpartyReviewAppealReviewAppealBundle.PriorBoardResponseAppeal == nil ||
		counterpartyReviewAppealReviewAppealBundle.PriorBoardResponseAppeal.Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusReversed ||
		len(counterpartyReviewAppealReviewAppealBundle.ReviewActions) != 3 ||
		len(counterpartyReviewAppealReviewAppealBundle.ReviewAppealActions) == 0 ||
		strings.TrimSpace(counterpartyReviewAppealReviewAppealBundle.CounterpartyReviewAppealReviewBundleHash) == "" ||
		strings.TrimSpace(counterpartyReviewAppealReviewAppealBundle.CounterpartyReviewAppealBundleHash) == "" {
	...
	acknowledgedCounterpartyReviewAppealReviewAppealBundle, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealAcknowledgeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            counterpartyReviewAppealReviewAppealSnapshots[0].SnapshotID,
		CounterpartyReference: "counterparty-review-appeal-review-appeal-ruling-ack-0001",
		Reason:                "acknowledge reciprocal imported-ruling review rehearing board bundle",
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-ACK-01"},
	})
	if err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealRuling failed: %v", err)
	}
	if !controlLedgerHasControl(acknowledgedCounterpartyReviewAppealReviewAppealBundle.ControlLedger, "CELL-FED-43") {
		t.Fatalf("expected acknowledged reciprocal imported-ruling review rehearing board control ledger to include CELL-FED-43, got %+v", acknowledgedCounterpartyReviewAppealReviewAppealBundle.ControlLedger.Controls)
	}

	disputedCounterpartyReviewAppealReviewAppealBundle, err := service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealDisputeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            counterpartyReviewAppealReviewAppealSnapshots[0].SnapshotID,
		CounterpartyReference: "counterparty-review-appeal-review-appeal-ruling-dispute-0001",
		Reason:                "dispute reciprocal imported-ruling review rehearing board bundle after divergence review",
		Divergences:           []string{"counterparty imported review rehearing board ruling diverged from prior local reversal evidence"},
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealRuling failed: %v", err)
	}
	if !controlLedgerHasControl(disputedCounterpartyReviewAppealReviewAppealBundle.ControlLedger, "CELL-FED-43") {
		t.Fatalf("expected disputed reciprocal imported-ruling review rehearing board control ledger to include CELL-FED-43, got %+v", disputedCounterpartyReviewAppealReviewAppealBundle.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewActions failed: %v", err)
	}
	if len(counterpartyReviewAppealReviewAppealReviewActions) != 2 ||
		counterpartyReviewAppealReviewAppealReviewActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewActionDispute ||
		counterpartyReviewAppealReviewAppealReviewActions[0].LocalReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewStatusDisputed ||
		counterpartyReviewAppealReviewAppealReviewActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewActionAcknowledge ||
		counterpartyReviewAppealReviewAppealReviewActions[1].LocalReviewStatus != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewStatusAcknowledged {
	...
	importedReciprocalReviewBundleResult, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   counterpartyReviewAppealReviewAppealReviewBundle,
		Reason:   "ingest reciprocal rehearing review bundle for bilateral governance",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-REVIEW-BUNDLE-INGEST-01"},
	})
	if err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundle failed: %v", err)
	}
	if !controlLedgerHasControl(importedReciprocalReviewBundleResult.ControlLedger, "CELL-FED-44") {
		t.Fatalf("expected reciprocal rehearing review bundle intake control ledger to include CELL-FED-44, got %+v", importedReciprocalReviewBundleResult.ControlLedger.Controls)
	}

	importedReciprocalReviewBundles, err := service.ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundles(ctx, SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleFilter{
		CellID:             created.CellID,
		ChallengeAppealID:  challengeAppealID,
		ResponseAppealID:   counterpartyReviewAppealReviewAppealSnapshots[0].ResponseAppealID,
		ReviewedSnapshotID: counterpartyReviewAppealReviewAppealSnapshots[0].SnapshotID,
	})
	if err != nil {
		t.Fatalf("ListFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundles failed: %v", err)
	}
	if len(importedReciprocalReviewBundles) != 1 ||
		!importedReciprocalReviewBundles[0].Verified ||
		importedReciprocalReviewBundles[0].Status != SecureCellFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealStatusVerified ||
		strings.TrimSpace(importedReciprocalReviewBundles[0].ContentHash) == "" {
		t.Fatalf("expected one verified imported reciprocal rehearing review bundle, got %+v", importedReciprocalReviewBundles)
	}

	storedReciprocalReviewBundle, err := service.GetFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundle(ctx, created.CellID, importedReciprocalReviewBundles[0].SnapshotID)
	if err != nil {
		t.Fatalf("GetFederationCounterpartyIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundle failed: %v", err)
	}
	if storedReciprocalReviewBundle == nil || !strings.EqualFold(strings.TrimSpace(storedReciprocalReviewBundle.ContentHash), strings.TrimSpace(counterpartyReviewAppealReviewAppealReviewBundle.ContentHash)) {
		t.Fatalf("expected stored reciprocal review bundle to preserve content hash, got %+v", storedReciprocalReviewBundle)
	}

	if _, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleAcknowledgeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            importedReciprocalReviewBundles[0].SnapshotID,
		CounterpartyReference: "counterparty-review-bundle-ack-0001",
		Reason:                "acknowledge imported reciprocal rehearing review bundle",
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-REVIEW-BUNDLE-ACK-01"},
	}); err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleRuling failed: %v", err)
	}

	disputedReciprocalReviewBundleResult, err := service.DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleRuling(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleDisputeRequest{
		ActorDID:              owner.AgentID(),
		SnapshotID:            importedReciprocalReviewBundles[0].SnapshotID,
		CounterpartyReference: "counterparty-review-bundle-dispute-0001",
		Reason:                "dispute imported reciprocal rehearing review bundle",
		Divergences:           []string{"review_bundle_resolution_scope"},
		Metadata:              map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-REVIEW-BUNDLE-DISPUTE-01"},
	})
	if err != nil {
		t.Fatalf("DisputeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleRuling failed: %v", err)
	}
	if !controlLedgerHasControl(disputedReciprocalReviewBundleResult.ControlLedger, "CELL-FED-45") {
		t.Fatalf("expected reciprocal rehearing review bundle governance control ledger to include CELL-FED-45, got %+v", disputedReciprocalReviewBundleResult.ControlLedger.Controls)
	}

	reciprocalReviewBundleActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  importedReciprocalReviewBundles[0].ResponseAppealID,
		SnapshotID:        importedReciprocalReviewBundles[0].SnapshotID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActions failed: %v", err)
	}
	if len(reciprocalReviewBundleActions) != 2 ||
		reciprocalReviewBundleActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionDispute ||
		reciprocalReviewBundleActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionAcknowledge {
		t.Fatalf("expected imported reciprocal review bundle local review trail with dispute then acknowledge lineage, got %+v", reciprocalReviewBundleActions)
	}

	escalatedReciprocalReviewBundleResult, err := service.EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleDispute(ctx, created.CellID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleDisputeEscalationRequest{
		ActorDID:                            owner.AgentID(),
		SnapshotID:                          importedReciprocalReviewBundles[0].SnapshotID,
		CounterpartyReference:               "counterparty-review-bundle-escalate-0001",
		AppealingParty:                      SecureCellFederationIncidentResponsePartyLocalOrg,
		CorrectionBoardParty:                SecureCellFederationIncidentResponsePartyLocalOrg,
		EnforcementAcknowledgementParty:     SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		Summary:                             "Escalate disputed imported reciprocal review bundle into governed rehearing",
		Description:                         "The local organization escalated the disputed imported reciprocal review bundle into a fresh signed reciprocal board.",
		EvidenceIDs:                         []string{"counterparty-review-bundle-escalation"},
		CorrectionBoardReviewThreshold:      1,
		EligibleCorrectionBoardReviewerDIDs: []string{owner.AgentID()},
		Reason:                              "escalate disputed imported reciprocal review bundle into governed rehearing",
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-REVIEW-BUNDLE-ESCALATE-01"},
	})
	if err != nil {
		t.Fatalf("EscalateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleDispute failed: %v", err)
	}
	if !controlLedgerHasControl(escalatedReciprocalReviewBundleResult.ControlLedger, "CELL-FED-45") {
		t.Fatalf("expected escalated reciprocal review bundle control ledger to include CELL-FED-45, got %+v", escalatedReciprocalReviewBundleResult.ControlLedger.Controls)
	}

	currentRun, err = service.getRun(created.CellID)
	if err != nil {
		t.Fatalf("getRun after reciprocal review bundle escalation failed: %v", err)
	}
	escalatedReciprocalReviewBundleAppeal := secureCellLatestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealForChallengeAppeal(currentRun, challengeAppealID)
	if escalatedReciprocalReviewBundleAppeal == nil ||
		!strings.EqualFold(strings.TrimSpace(escalatedReciprocalReviewBundleAppeal.Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_counterparty_review_appeal_review_appeal_review_bundle_escalated"]), "true") ||
		!strings.EqualFold(strings.TrimSpace(escalatedReciprocalReviewBundleAppeal.Metadata["federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_counterparty_review_appeal_review_appeal_review_bundle_snapshot_id"]), strings.TrimSpace(importedReciprocalReviewBundles[0].SnapshotID)) ||
		!strings.EqualFold(strings.TrimSpace(escalatedReciprocalReviewBundleAppeal.ParentResponseAppealID), strings.TrimSpace(importedReciprocalReviewBundles[0].LocalBoardResponseAppealID)) {
		t.Fatalf("expected latest appeal to be the escalated reciprocal review bundle board, got %+v", escalatedReciprocalReviewBundleAppeal)
	}

	resolvedReciprocalReviewBundleResult, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, escalatedReciprocalReviewBundleAppeal.ResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             owner.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify,
		RulingSummary:        "Resolve disputed imported reciprocal review bundle through governed rehearing",
		RulingDescription:    "The fresh reciprocal board closed the disputed imported review-bundle loop with signed evidence.",
		EvidenceIDs:          []string{"counterparty-review-bundle-resolution"},
		Reason:               "resolve disputed imported reciprocal review bundle through governed rehearing",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-APPEAL-COUNTERPARTY-REVIEW-APPEAL-REVIEW-APPEAL-REVIEW-BUNDLE-RESOLVE-01"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal for reciprocal review bundle escalation failed: %v", err)
	}
	if !controlLedgerHasControl(resolvedReciprocalReviewBundleResult.ControlLedger, "CELL-FED-45") {
		t.Fatalf("expected resolved reciprocal review bundle control ledger to include CELL-FED-45, got %+v", resolvedReciprocalReviewBundleResult.ControlLedger.Controls)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActions after escalation failed: %v", err)
	}
	if len(reciprocalReviewBundleActions) != 4 ||
		reciprocalReviewBundleActions[0].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionResolve ||
		reciprocalReviewBundleActions[1].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionEscalate ||
		reciprocalReviewBundleActions[2].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionDispute ||
		reciprocalReviewBundleActions[3].Action != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealCounterpartyReviewAppealReviewAppealReviewBundleReviewActionAcknowledge {
		t.Fatalf("expected imported reciprocal review bundle trail with resolve, escalate, dispute, acknowledge lineage, got %+v", reciprocalReviewBundleActions)
	}
	...
	if !controlLedgerHasControl(appealedAlignmentResponse.ControlLedger, "CELL-FED-32") ||
		...
		!controlLedgerHasControl(ruledAlignmentResponseAppeal.ControlLedger, "CELL-FED-32") ||
		!controlLedgerHasControl(acknowledgedAlignmentResponseAppeal.ControlLedger, "CELL-FED-32") {
		t.Fatalf("expected alignment response appeal control ledgers to include CELL-FED-32")
	}

	realignedChallengeAppealBundle, err := service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
	...
	if _, err := service.IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, organizationID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleIntakeRequest{
		ActorDID: owner.AgentID(),
		Bundle:   realignedChallengeAppealBundle,
		Reason:   "ingest realigned reciprocal challenge appeal bundle for automated correction-board supervision",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-INGEST-01"},
	}); err != nil {
		t.Fatalf("IngestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle for automated response appeal failed: %v", err)
	}

	if _, err := service.AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentAcknowledgeRequest{
		ActorDID: owner.AgentID(),
		Reason:   "acknowledge realigned reciprocal challenge appeal bundle for automated correction-board supervision",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-ACK-01"},
	}); err != nil {
		t.Fatalf("AcknowledgeFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignment for automated response appeal failed: %v", err)
	}

	if _, err := service.AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrection(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrectionAttestationRequest{
		ActorDID:               owner.AgentID(),
		CounterpartySnapshotID: counterpartyChallengeAppeals[0].SnapshotID,
		CounterpartyReference:  "counterparty-alignment-response-appeal-automation-correct-0001",
		Reason:                 "attest corrected handling for automated correction-board response appeal supervision",
		Metadata:               map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-CORRECT-01"},
	}); err != nil {
		t.Fatalf("AttestFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentCorrection for automated response appeal failed: %v", err)
	}

	if _, err := service.AppealFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponse(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRequest{
		ActorDID:                            owner.AgentID(),
		AppealingParty:                      SecureCellFederationIncidentResponsePartyLocalOrg,
		CorrectionBoardParty:                SecureCellFederationIncidentResponsePartyLocalOrg,
		EnforcementAcknowledgementParty:     SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		Summary:                             "Local org appealed corrected alignment response under automated supervision",
		Description:                         "The local organization escalated the corrected reciprocal challenge appeal alignment response into a timed correction-board path.",
		EvidenceIDs:                         []string{directiveID, comparisonKey, challengeAppealID},
		CorrectionBoardReviewThreshold:      2,
		EligibleCorrectionBoardReviewerDIDs: []string{participantA.AgentID()},
		Reason:                              "open automated correction-board appeal over corrected alignment response",
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-OPEN-01"},
	}); err != nil {
		t.Fatalf("AppealFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponse for automated supervision failed: %v", err)
	}

	automatedAlignmentResponseAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals automated pending failed: %v", err)
	}
	if len(automatedAlignmentResponseAppeals) != 1 || automatedAlignmentResponseAppeals[0].BoardReviewThreshold != 2 || automatedAlignmentResponseAppeals[0].BoardMissingQuorumCount != 2 {
		t.Fatalf("expected one pending automated alignment response appeal with missing quorum, got %+v", automatedAlignmentResponseAppeals)
	}
	automatedResponseAppealID := automatedAlignmentResponseAppeals[0].ResponseAppealID
	responseAppealReviewOverdueAt := automatedAlignmentResponseAppeals[0].CreatedAt.Add(25 * time.Hour)

	overdueAutomatedResponseAppeals, err := service.ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  automatedResponseAppealID,
		Before:            &responseAppealReviewOverdueAt,
	})
	if err != nil {
		t.Fatalf("ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals review failed: %v", err)
	}
	if len(overdueAutomatedResponseAppeals) != 1 || overdueAutomatedResponseAppeals[0].PendingAction != "board_review" || overdueAutomatedResponseAppeals[0].AutomationAction != "delegate_review_committee" {
		t.Fatalf("expected one overdue automated alignment response appeal board-review item, got %+v", overdueAutomatedResponseAppeals)
	}

	if _, err := service.SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, responseAppealReviewOverdueAt, SecureCellLifecycleRequest{
		ActorDID: challengeAppealSweepActor,
		Reason:   "automated alignment response appeal sweep",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-SWEEP-01"},
	}); err != nil {
	...
	responseAppealAutomationActions, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActions(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActionFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  automatedResponseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActions after review sweep failed: %v", err)
	}
	if len(responseAppealAutomationActions) != 1 || responseAppealAutomationActions[0].PendingAction != "board_review" || responseAppealAutomationActions[0].Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_review_delegated" || responseAppealAutomationActions[0].AutomatedActor != challengeAppealSweepActor {
		t.Fatalf("expected one automated correction-board delegation action, got %+v", responseAppealAutomationActions)
	}
	automatedAlignmentResponseAppeals, err = service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after review sweep failed: %v", err)
	}
	if len(automatedAlignmentResponseAppeals) != 1 {
		t.Fatalf("expected one delegated automated alignment response appeal, got %+v", automatedAlignmentResponseAppeals)
	}
	automatedCorrectionBoardReviewer := ""
	for _, reviewer := range automatedAlignmentResponseAppeals[0].EligibleBoardReviewerDIDs {
	...
	if strings.TrimSpace(automatedCorrectionBoardReviewer) == "" {
		t.Fatalf("expected delegated automated correction-board reviewer, got %+v", automatedAlignmentResponseAppeals[0])
	}

	if _, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, automatedResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             participantA.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify,
		RulingSummary:        "First correction-board vote ratified the acknowledged alignment response",
		RulingDescription:    "The first correction-board reviewer favored ratifying the acknowledged reciprocal alignment response, but the threshold is not satisfied yet.",
		EvidenceIDs:          []string{directiveID, comparisonKey, automatedResponseAppealID},
		Reason:               "record first automated correction-board vote",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-VOTE-01"},
	}); err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal first automated vote failed: %v", err)
	}

	ruledAutomatedAlignmentResponseAppeal, err := service.RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, automatedResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRulingRequest{
		ActorDID:             automatedCorrectionBoardReviewer,
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Ruling:               SecureCellFederationIncidentDirectiveExtensionAppealRulingRatify,
		RulingSummary:        "Correction-board ratified acknowledged alignment response",
		RulingDescription:    "The delegated reviewer completed the correction-board quorum and ratified the corrected reciprocal alignment response.",
		EvidenceIDs:          []string{directiveID, comparisonKey, automatedResponseAppealID},
		Reason:               "complete automated correction-board ruling",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-VOTE-02"},
	})
	if err != nil {
		t.Fatalf("RuleFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal final automated vote failed: %v", err)
	}

	ruledAutomatedAlignmentResponseAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  automatedResponseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals automated ruled failed: %v", err)
	}
	if len(ruledAutomatedAlignmentResponseAppeals) != 1 || ruledAutomatedAlignmentResponseAppeals[0].Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusUpheld || ruledAutomatedAlignmentResponseAppeals[0].RatifyVoteCount != 2 {
		t.Fatalf("expected one upheld automated alignment response appeal, got %+v", ruledAutomatedAlignmentResponseAppeals)
	}
	responseAppealAckOverdueAt := ruledAutomatedAlignmentResponseAppeals[0].RuledAt.Add(13 * time.Hour)

	overdueAutomatedResponseAppeals, err = service.ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  automatedResponseAppealID,
		Before:            &responseAppealAckOverdueAt,
	})
	if err != nil {
		t.Fatalf("ListOverdueFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals acknowledgement failed: %v", err)
	}
	if len(overdueAutomatedResponseAppeals) != 1 || overdueAutomatedResponseAppeals[0].PendingAction != "acknowledge_enforcement" || (overdueAutomatedResponseAppeals[0].AutomationAction != "" && overdueAutomatedResponseAppeals[0].AutomationAction != "escalate_response" && overdueAutomatedResponseAppeals[0].AutomationAction != "suspend_contracts") {
		t.Fatalf("expected one overdue automated alignment response appeal acknowledgement item, got %+v", overdueAutomatedResponseAppeals)
	}
	expectedAutomatedEnforcementRecovery := strings.TrimSpace(overdueAutomatedResponseAppeals[0].AutomationAction) != ""

	if _, err := service.SweepFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, responseAppealAckOverdueAt, SecureCellLifecycleRequest{
		ActorDID: challengeAppealSweepActor,
		Reason:   "automated alignment response appeal acknowledgement sweep",
		Metadata: map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-AUTO-SWEEP-02"},
	}); err != nil {
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActions final failed: %v", err)
	}
	foundAutomatedCommitteeRecovery := false
	foundAutomatedEnforcementRecovery := false
	for _, item := range responseAppealAutomationActions {
		if item.AutomatedActor != challengeAppealSweepActor {
			t.Fatalf("expected automated actor %s, got %+v", challengeAppealSweepActor, responseAppealAutomationActions)
		}
		switch item.PendingAction {
		case "board_review":
			if item.Action != "secure_cell.federation_incident_directive_extension_appeal_reconciliation_challenge_appeal_alignment_response_appeal_review_delegated" {
				t.Fatalf("expected automated board-review delegation action, got %+v", responseAppealAutomationActions)
			}
			foundAutomatedCommitteeRecovery = true
		case "acknowledge_enforcement":
			if item.Action != "secure_cell.federation_incident_response_escalated" && item.Action != "secure_cell.federation_contract_suspended" {
				t.Fatalf("expected automated acknowledgement recovery to escalate or suspend, got %+v", responseAppealAutomationActions)
			}
	...
	if !foundAutomatedCommitteeRecovery || (expectedAutomatedEnforcementRecovery && !foundAutomatedEnforcementRecovery) {
		t.Fatalf("expected automated alignment response appeal supervision to cover review and acknowledgement, got %+v", responseAppealAutomationActions)
	}
	...
	alignmentResponseBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseBundle after automated response appeal failed: %v", err)
	}
	if len(alignmentResponseBundle.ResponseAppealAutomationActions) < expectedAutomationActionCount {
		t.Fatalf("expected alignment response bundle to include automated correction-board supervision, got %+v", alignmentResponseBundle.ResponseAppealAutomationActions)
	}

	challengeAppealBundle, err = service.BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle(ctx, created.CellID, challengeAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundleOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealBundle after automated response appeal failed: %v", err)
	}
	if len(challengeAppealBundle.CounterpartyAlignmentResponseAppealAutomationActions) < expectedAutomationActionCount {
		t.Fatalf("expected challenge appeal bundle to include automated alignment response appeal supervision, got %+v", challengeAppealBundle.CounterpartyAlignmentResponseAppealAutomationActions)
	}

	casePack, err = service.BuildFederationIncidentCasePack(ctx, created.CellID, localResponseID, SecureCellFederationIncidentCasePackOptions{})
	if err != nil {
		t.Fatalf("BuildFederationIncidentCasePack after automated response appeal failed: %v", err)
	}
	if len(casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActions) < expectedAutomationActionCount {
		t.Fatalf("expected case pack to include automated alignment response appeal supervision, got %+v", casePack.DirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealAutomationActions)
	}
	if !controlLedgerHasControl(ruledAutomatedAlignmentResponseAppeal.ControlLedger, "CELL-FED-33") {
		t.Fatalf("expected automated alignment response appeal control ledger to include CELL-FED-33, got %+v", ruledAutomatedAlignmentResponseAppeal.ControlLedger.Controls)
	}
	currentAfterAutomatedAlignmentResponseAppealSweep, err := service.GetCell(ctx, created.CellID)
	if err != nil {
		t.Fatalf("GetCell after automated alignment response appeal sweep failed: %v", err)
	}
	if !controlLedgerHasControl(currentAfterAutomatedAlignmentResponseAppealSweep.ControlLedger, "CELL-FED-33") {
		t.Fatalf("expected current control ledger to include CELL-FED-33 after automated alignment response appeal sweep, got %+v", currentAfterAutomatedAlignmentResponseAppealSweep.ControlLedger.Controls)
	}

	reheardResponseAppeal, err := service.RehearFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal(ctx, created.CellID, automatedResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRehearingRequest{
		ActorDID:                            participantB.AgentID(),
		AppealingParty:                      SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		CorrectionBoardParty:                SecureCellFederationIncidentResponsePartyLocalOrg,
		EnforcementAcknowledgementParty:     SecureCellFederationIncidentResponsePartyCounterpartyOrg,
		Summary:                             "Counterparty requested response-appeal rehearing",
		Description:                         "The counterparty requested a fresh correction-board response appeal after the acknowledged ruling.",
		EvidenceIDs:                         []string{directiveID, comparisonKey, automatedResponseAppealID},
		CorrectionBoardReviewThreshold:      1,
		EligibleCorrectionBoardReviewerDIDs: []string{participantA.AgentID()},
		Reason:                              "request response-appeal rehearing",
		Metadata:                            map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-REHEAR-01"},
	})
	if err != nil {
		t.Fatalf("RehearFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeal failed: %v", err)
	}

	rehearingResponseAppeals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealFilter{
		CellID:                 created.CellID,
		ChallengeAppealID:      challengeAppealID,
		ParentResponseAppealID: automatedResponseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals rehearings failed: %v", err)
	}
	if len(rehearingResponseAppeals) != 1 || rehearingResponseAppeals[0].ParentResponseAppealID != automatedResponseAppealID || rehearingResponseAppeals[0].ResponseAppealGeneration != 2 || rehearingResponseAppeals[0].Status != SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealStatusPendingCorrectionBoardReview {
		t.Fatalf("expected one pending response-appeal rehearing, got %+v", rehearingResponseAppeals)
	}
	rehearingResponseAppealID := rehearingResponseAppeals[0].ResponseAppealID

	recusedResponseAppeal, err := service.RecuseFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview(ctx, created.CellID, rehearingResponseAppealID, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecuseRequest{
		ActorDID:             participantA.AgentID(),
		CorrectionBoardParty: SecureCellFederationIncidentResponsePartyLocalOrg,
		Summary:              "Alternate reviewer recused from response-appeal rehearing",
		Description:          "The alternate reviewer declared a conflict and stepped off the response-appeal correction board.",
		EvidenceIDs:          []string{directiveID, comparisonKey, rehearingResponseAppealID},
		Reason:               "recuse conflicted response-appeal rehearing reviewer",
		Metadata:             map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-RECUSE-01"},
	})
	if err != nil {
		t.Fatalf("RecuseFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview failed: %v", err)
	}

	responseAppealRecusals, err := service.ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecusals(ctx, SecureCellFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecusalFilter{
		CellID:            created.CellID,
		ChallengeAppealID: challengeAppealID,
		ResponseAppealID:  rehearingResponseAppealID,
	})
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealRecusals failed: %v", err)
	}
	if len(responseAppealRecusals) != 1 || responseAppealRecusals[0].ActorDID != participantA.AgentID() || responseAppealRecusals[0].ResponseAppealID != rehearingResponseAppealID {
		t.Fatalf("expected one response-appeal rehearing recusal, got %+v", responseAppealRecusals)
	}

	delegatedRehearingResponseAppeal, err := service.DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview(ctx, created.CellID, rehearingResponseAppealID, SecureCellFederationIncidentDirectiveExtensionDelegationRequest{
		ActorDID:  owner.AgentID(),
		TargetDID: owner.AgentID(),
		Reason:    "delegate replacement response-appeal rehearing reviewer",
		Metadata:  map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-ALIGN-RESP-DELEGATE-01"},
	})
	if err != nil {
		t.Fatalf("DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppealReview failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("ListFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealAlignmentResponseAppeals after recusal failed: %v", err)
	}
	if len(rehearingResponseAppeals) != 1 || rehearingResponseAppeals[0].BoardRecusalCount != 1 || rehearingResponseAppeals[0].BoardDelegationCount != 1 || rehearingResponseAppeals[0].ResponseAppealGeneration != 2 || !containsStringFold(rehearingResponseAppeals[0].EligibleBoardReviewerDIDs, owner.AgentID()) {
		t.Fatalf("expected recusal-aware response-appeal rehearing posture, got %+v", rehearingResponseAppeals)
	}
	...
	for _, item := range alignmentResponseBundle.ResponseAppeals {
		if item.ResponseAppealID == rehearingResponseAppealID && item.ParentResponseAppealID == automatedResponseAppealID && item.ResponseAppealGeneration == 2 && item.BoardDelegationCount == 1 {
			foundRehearingResponseAppeal = true
			break
		}
	}
	if !foundRehearingResponseAppeal {
		t.Fatalf("expected alignment response bundle to include response-appeal rehearing lineage, got %+v", alignmentResponseBundle.ResponseAppeals)
	}
	...
	if !controlLedgerHasControl(recusedResponseAppeal.ControlLedger, "CELL-FED-36") {
		t.Fatalf("expected recused response appeal control ledger to include CELL-FED-36, got %+v", recusedResponseAppeal.ControlLedger.Controls)
	}
	if !controlLedgerHasControl(delegatedRehearingResponseAppeal.ControlLedger, "CELL-FED-36") {
		t.Fatalf("expected delegated response appeal control ledger to include CELL-FED-36, got %+v", delegatedRehearingResponseAppeal.ControlLedger.Controls)
	}
	if !controlLedgerHasControl(reheardResponseAppeal.ControlLedger, "CELL-FED-36") {
		t.Fatalf("expected reheard response appeal control ledger to include CELL-FED-36, got %+v", reheardResponseAppeal.ControlLedger.Controls)
	}
	...
	delegatedRehearingChallengeAppeal, err := service.DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealReview(ctx, created.CellID, rehearingChallengeAppealID, SecureCellFederationIncidentDirectiveExtensionDelegationRequest{
		ActorDID:  owner.AgentID(),
		TargetDID: owner.AgentID(),
		Reason:    "delegate replacement reconciliation challenge appeal rehearing reviewer",
		Metadata:  map[string]string{"ticket": "FED-DIRECTIVE-EXT-APPEAL-RECON-CHALLENGE-APPEAL-DELEGATE-01"},
	})
	if err != nil {
		t.Fatalf("DelegateFederationIncidentDirectiveExtensionAppealReconciliationChallengeAppealReview failed: %v", err)
	}
	...
	if len(rehearingChallengeAppeals) != 1 || rehearingChallengeAppeals[0].BoardRecusalCount != 1 || rehearingChallengeAppeals[0].BoardDelegationCount != 1 || rehearingChallengeAppeals[0].ChallengeAppealGeneration != 2 || !containsStringFold(rehearingChallengeAppeals[0].EligibleBoardReviewerDIDs, owner.AgentID()) {
	...
	if reconciliationBundle.ChallengeAppeals[0].BoardDelegationCount == 0 && reconciliationBundle.ChallengeAppeals[1].BoardDelegationCount == 0 {
		t.Fatalf("expected reconciliation bundle to include challenge appeal delegation posture, got %+v", reconciliationBundle.ChallengeAppeals)
	}
	...
	if !controlLedgerHasControl(delegatedRehearingChallengeAppeal.ControlLedger, "CELL-FED-27") {
		t.Fatalf("expected delegated challenge appeal control ledger to include CELL-FED-27, got %+v", delegatedRehearingChallengeAppeal.ControlLedger.Controls)
	}
