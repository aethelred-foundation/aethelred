...
done < <(rg -n 'SetConsensusThresholdForTesting' "$POUW_DIR" -g'*.go')

# M-05: deterministic + entropy-salted tie-break path must remain in production selection logic.
rg -q 'selectionSeed := vs\.selectionEntropySeed' "$STAKING_FILE" \
  || fail "M-05 guard missing selectionEntropySeed tie-break seeding"
rg -q 'selectionTieBreaker\(selectionSeed, candidate\.Address\)' "$STAKING_FILE" \
  || fail "M-05 guard missing per-validator entropy tie-breaker"
rg -q 'bytes\.Compare\(ib\[:\], jb\[:\]\)' "$STAKING_FILE" \
  || fail "M-05 guard missing deterministic tie-break comparison"
rg -q 'return candidates\[i\]\.Address < candidates\[j\]\.Address' "$STAKING_FILE" \
