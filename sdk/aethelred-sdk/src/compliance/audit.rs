...
impl AuditTrail {
    ...
    pub fn generate_report(&self, config: &ReportConfig) -> AuditReport {
        ...
        let allowed_operations = entries.iter().filter(|e| e.result == AuditResult::Allowed).count();
        let denied_operations = entries.iter().filter(|e| e.result == AuditResult::Denied).count();

        // Count violations by type
        let mut violations_by_type = std::collections::HashMap::new();
        for entry in &entries {
            for violation in &entry.violations {
                *violations_by_type.entry(violation.code.clone()).or_insert(0) += 1;
            }
        }
...
impl AuditReport {
    ...
    pub fn summary(&self) -> String {
        ...
        summary.push_str(&format!("Allowed: {} ({:.1}%)\n",
...
impl AuditBackend {
    ...
    fn persist(&self, _entry: &AuditEntry) {
        ...
        match self {
            ...
            AuditBackend::Database { connection_string: _ } => {
