...
impl ZkTensor {
    ...
    /// Softmax activation
    pub fn softmax(&self, dim: i32) -> ZkTensor {
        // Simplified 1D softmax
        let max_val = self.data.values.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
        let exp_values: Vec<f32> = self.data.values.iter().map(|&x| (x - max_val).exp()).collect();
    ...
    pub fn verify(&self) -> Result<bool, ZkTensorError> {
        let proof = self.proof.as_ref().ok_or(ZkTensorError::NoProof)?;
...
impl ProofVerifier {
    ...
    pub fn verify(&self, proof: &ExportedProof) -> Result<bool, ZkTensorError> {
        let _vk = self.circuits.get(&proof.circuit_hash).ok_or_else(|| {
            ZkTensorError::UnknownCircuit(hex::encode(proof.circuit_hash))
        })?;
