                ...
                if let (Some(from), Some(to)) = (&rule.source, &rule.destination) {
                    if *from == Jurisdiction::EU || from.is_part_of(&Jurisdiction::EU) {
                        if !Jurisdiction::EU.has_adequacy_with(to) {
                            ...
                            if !rule
                                .safeguards
                                .contains(&TransferSafeguard::StandardContractualClauses)
                            {
                                issues.push(LintIssue {
                ...
                if j1 != j2 {
                        ...
                        && j2.regulations().iter().any(|r| r.requires_localization())
                    {
                        issues.push(LintIssue {
                            ...
                            location: None,
                            suggestion: "Consider separate data stores for each jurisdiction"
                                .to_string(),
                            legal_reference: None,
                        });
                    }
...
pub enum LintRuleSet {
    /// Data residency and localization checks.
    DataLocalization,
    /// Cross-border transfer rule checks.
    TransferRules,
    /// Retention/expiry policy checks.
    RetentionPolicies,
    /// Encryption requirement checks.
    EncryptionRequirements,
    /// Consent capture and consent-use checks.
    ConsentManagement,
    /// Audit trail presence/quality checks.
    AuditTrail,
    /// Broader cross-border policy checks.
    CrossBorder,
    /// Data classification consistency checks.
    DataClassification,
}
...
pub struct TransferRule {
    /// Rule name/identifier for reporting and diagnostics.
    pub name: String,
    /// Optional source jurisdiction selector (None = any).
    pub source: Option<Jurisdiction>,
    /// Optional destination jurisdiction selector (None = any).
    pub destination: Option<Jurisdiction>,
    /// Safeguards required when this rule matches.
    pub safeguards: Vec<TransferSafeguard>,
    /// Whether explicit data subject consent is required.
...
pub enum TransferSafeguard {
    ...
    /// Adequacy decision from regulator.
    AdequacyDecision,
    /// Explicit subject consent for transfer.
    ExplicitConsent,
    /// Contract necessity exception.
    ContractNecessity,
    /// Public-interest legal basis.
    PublicInterest,
    /// Legal claims / legal proceedings exception.
    LegalClaims,
}
...
mod tests {
    ...
    fn test_gdpr_transfer_violation() {
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.code == "GDPR_TRANSFER_SAFEGUARD"));
    }
    ...
    fn test_retention_violation() {
        ...
        retention.insert(
            "logs".to_string(),
            Duration::from_secs(365 * 24 * 60 * 60 * 20),
        ); // 20 years
