...
pub enum USState {
    ...
    /// Utah (UCPA).
    Utah, // UCPA
    /// Texas privacy law regime.
    Texas,
    /// New York state-specific compliance regime.
    NewYork,
    /// Any other US state not explicitly enumerated.
    Other,
}
...
impl Jurisdiction {
    ...
    /// Conservative export-allowance check used by legacy sovereign APIs.
    pub fn can_export_to(&self, other: Jurisdiction) -> bool {
        self == &other || self.has_adequacy_with(&other) || other.is_part_of(self)
    }
}
...
/// Backward-compatible alias used by older sovereign/prelude surfaces.
pub type CompliancePolicy = ComplianceRequirement;
...
impl ComplianceEngine {
    ...
    pub fn check_transfer(
        ...
        self.transfer_rules
            .check(from, to, regulations, &self.regulation_policies)
    }

    /// Check compliance for a data operation
    pub fn check_operation(&self, operation: &DataOperation) -> ComplianceResult {
    ...
    fn check_jurisdiction_rules(&self, operation: &DataOperation) -> Option<ComplianceViolation> {
        ...
        if !self
            .jurisdiction_rules
            .allows(operation.jurisdiction, &operation.operation_type)
        {
        ...
        if policy.data_localization {
            if !policy.allowed_transfers.contains(&operation.jurisdiction) {
                return Some(ComplianceViolation {
                    ...
                    message: format!("Data must remain in: {:?}", policy.allowed_transfers),
                    severity: ViolationSeverity::Critical,
...
pub struct RegulationPolicy {
    /// Regulation this policy describes.
    pub regulation: Regulation,
    /// Whether data must remain localized within jurisdiction.
    pub data_localization: bool,
    /// Allowed transfer destinations under this policy.
    pub allowed_transfers: Vec<Jurisdiction>,
    /// Whether encryption is mandatory for regulated data.
    pub require_encryption: bool,
    /// Whether audit logging is mandatory.
    pub require_audit_log: bool,
    /// Whether explicit consent is required for processing.
    pub require_consent: bool,
    /// Maximum retention duration permitted by policy, if specified.
    pub max_retention: Option<Duration>,
    /// Breach notification deadline in hours, if mandated.
...
pub struct DataOperation {
    /// Operation label/type (e.g., read, write, transfer).
    pub operation_type: String,
    /// Jurisdiction where the operation executes.
    pub jurisdiction: Jurisdiction,
    /// Regulations applicable to the operation.
    pub applicable_regulations: Vec<Regulation>,
    /// Data classification for the operated dataset.
    pub data_classification: DataClassification,
    /// Whether payload/data at rest is encrypted.
    pub is_encrypted: bool,
    /// Whether audit logging is enabled for this operation.
    pub audit_enabled: bool,
    /// Whether TEE attestation was verified for the execution environment.
    pub tee_verified: bool,
    /// Actor identity (service/user/system principal).
    pub actor: String,
}
...
pub enum DataClassification {
    /// Public data with no confidentiality restrictions.
    Public,
    /// Internal operational data.
    Internal,
    /// Confidential business data.
    Confidential,
    /// Sensitive regulated or customer data.
    Sensitive,
...
pub struct ComplianceResult {
    ...
    pub compliant: bool,
    /// Violations that block or downgrade compliance.
    pub violations: Vec<ComplianceViolation>,
    /// Advisory warnings that do not necessarily block execution.
    pub warnings: Vec<ComplianceWarning>,
    /// Audit identifier associated with the compliance decision.
    pub audit_id: String,
}
...
pub struct ComplianceViolation {
    /// Stable violation code for tooling and dashboards.
    pub code: String,
    /// Human-readable violation description.
    pub message: String,
    /// Violation severity level.
    pub severity: ViolationSeverity,
    /// Triggering regulation, if attributable.
    pub regulation: Option<Regulation>,
    /// Suggested remediation to resolve the violation.
...
pub struct ComplianceWarning {
    /// Stable warning code for tooling and dashboards.
    pub code: String,
    /// Human-readable warning description.
    pub message: String,
    /// Warning severity level.
    pub severity: WarningSeverity,
    /// Recommended action to reduce risk or improve posture.
...
pub struct ComplianceConfig {
    /// Jurisdictions enabled in the deployment/profile.
    pub jurisdictions: Vec<Jurisdiction>,
    /// Regulations that should be enforced by linting.
    pub regulations: Vec<Regulation>,
    /// Data type/category labels referenced by the configuration.
    pub data_types: Vec<String>,
    /// Cross-border transfer rules used by the linter.
    pub transfer_rules: Vec<TransferRule>,
    /// Retention policy map keyed by data type/category.
