    ...
    pub fn default() -> Self {
        ...
        let eu_adequate = vec![
            (
                ...
                "Adequacy Decision 2000/518/EC",
            ),
            (
                Jurisdiction::EU,
                Jurisdiction::Japan,
                "Adequacy Decision 2019/419",
            ),
            (
                Jurisdiction::EU,
                Jurisdiction::UK,
                "Adequacy Decision 2021/1772",
            ),
            (
                Jurisdiction::EU,
                Jurisdiction::Canada,
                "Adequacy Decision 2002/2/EC",
            ),
            (
                Jurisdiction::EU,
                Jurisdiction::SouthKorea,
                "Adequacy Decision 2022",
            ),
        ];
        ...
        let gcc_countries = vec![Jurisdiction::UAE, Jurisdiction::SaudiArabia];
...
pub enum TransferResult {
    /// Transfer is allowed
    Allowed {
        /// Legal transfer mechanism authorizing the route.
        mechanism: TransferMechanism,
        /// Conditions that must be met for continued compliance.
        conditions: Vec<String>,
    },

    /// Transfer is blocked
    Blocked {
        /// Human-readable reason the transfer is blocked.
        reason: String,
        /// Triggering regulation, if one is directly attributable.
        regulation: Option<Regulation>,
    },

    /// Transfer requires additional safeguards
    RequiresSafeguards {
        /// Acceptable safeguards/mechanisms that can unblock the transfer.
        available_mechanisms: Vec<TransferMechanism>,
        /// Regulation imposing the safeguard requirement.
...
impl TransferValidator {
    ...
    pub fn validate(&self, request: &TransferRequest) -> TransferValidation {
        ...
        match result {
            ...
            TransferResult::RequiresSafeguards {
...
pub struct TransferRequest {
    /// Jurisdiction where the data currently resides.
    pub source: Jurisdiction,
    /// Requested destination jurisdiction.
    pub destination: Jurisdiction,
    /// Logical data categories involved in the transfer.
    pub data_types: Vec<String>,
    /// Regulations that apply to the transfer.
    pub regulations: Vec<Regulation>,
    /// Declared processing purpose for the transfer.
    pub purpose: String,
}
...
pub struct TransferValidation {
    /// Whether the requested transfer is currently permitted.
    pub allowed: bool,
    /// Transfer mechanism that makes the transfer lawful, if any.
    pub mechanism: Option<TransferMechanism>,
    /// Conditions that must hold during/after transfer.
    pub conditions: Vec<String>,
    /// Violations preventing approval.
    pub violations: Vec<TransferViolation>,
    /// Remediation steps required to unblock transfer.
...
pub struct TransferViolation {
    /// Stable violation code for policy tooling and dashboards.
    pub code: String,
    /// Human-readable reason for the violation.
    pub reason: String,
    /// Regulation associated with the violation, if identifiable.
...
mod tests {
    ...
    fn test_same_jurisdiction() {
        ...
        let result = rules.check(Jurisdiction::UAE, Jurisdiction::UAE, &[], &HashMap::new());
