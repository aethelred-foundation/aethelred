...
pub enum DilithiumSecurityLevel {
    /// Dilithium Level 2 (smaller keys/signatures, lower PQ security margin).
    Level2,
    #[default]
    /// Dilithium Level 3 (default balanced setting).
    Level3,
    /// Dilithium Level 5 (largest keys/signatures, highest PQ security margin).
    Level5,
}
...
pub struct HybridPublicKey {
    ...
    /// Selected Dilithium security level for this public key.
    #[cfg_attr(not(feature = "full-sdk"), serde(default))]
...
pub struct HybridSignature {
    ...
    pub dilithium: Vec<u8>,
    /// Selected Dilithium security level used when producing this signature.
    #[cfg_attr(not(feature = "full-sdk"), serde(default))]
    pub level: DilithiumSecurityLevel,
}
...
mod tests {
    ...
    #[test]
    fn test_bincode_roundtrip_hybrid_types() {
        let keypair = HybridKeypair::generate();
        let public = keypair.public_key();
        let signature = keypair.sign(b"serde roundtrip");

        let public_bytes = bincode::serialize(&public).expect("serialize public");
        let public_decoded: HybridPublicKey =
            bincode::deserialize(&public_bytes).expect("deserialize public");
        assert_eq!(public_decoded.ecdsa, public.ecdsa);
        assert_eq!(public_decoded.dilithium, public.dilithium);
        assert_eq!(public_decoded.level, public.level);

        let sig_bytes = bincode::serialize(&signature).expect("serialize signature");
        let sig_decoded: HybridSignature =
            bincode::deserialize(&sig_bytes).expect("deserialize signature");
        assert_eq!(sig_decoded.ecdsa, signature.ecdsa);
        assert_eq!(sig_decoded.dilithium, signature.dilithium);
        assert_eq!(sig_decoded.level, signature.level);
    }
}
