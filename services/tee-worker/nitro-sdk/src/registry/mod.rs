...
impl ModelRegistry {
    ...
    pub fn register(&mut self, model: ModelRegistration) -> Result<ModelId, RegistryError> {
    ...
    pub fn search(&self, query: &str) -> Vec<&ModelMetadata> {
        self.cache
            .values()
            .filter(|m| {
                m.name.contains(query) || m.description.contains(query) || m.author.contains(query)
            })
            .collect()
    }
