...
use crate::crypto::{HybridKeypair, HybridPublicKey, HybridSignature};
...
impl SealBuilder {
    ...
    pub fn build(self, keypair: &HybridKeypair) -> Result<DigitalSeal, SealError> {
        let model_commitment = self
            .model_commitment
            .ok_or(SealError::MissingField("model"))?;
        let input_commitment = self
            .input_commitment
            .ok_or(SealError::MissingField("input"))?;
        let output_commitment = self
            .output_commitment
            .ok_or(SealError::MissingField("output"))?;
        let attestation = self
            .attestation
            .ok_or(SealError::MissingField("attestation"))?;
...
impl DigitalSeal {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        out.extend_from_slice(&self.model_commitment);
        out.extend_from_slice(&self.input_commitment);
        out.extend_from_slice(&self.output_commitment);
        out.push(self.attestation.hardware as u8);
        out.extend_from_slice(&self.attestation.report.to_canonical_bytes());
        out.extend_from_slice(&(self.attestation.quote.len() as u64).to_le_bytes());
        out.extend_from_slice(&self.attestation.quote);
        ...
        out.extend_from_slice(&self.metadata.block_height.unwrap_or_default().to_le_bytes());
        out.extend_from_slice(self.metadata.jurisdiction.code().as_bytes());
        out.extend_from_slice(self.metadata.purpose.as_bytes());

        out.extend_from_slice(&(self.metadata.validators.len() as u64).to_le_bytes());
        for v in &self.metadata.validators {
...
pub enum SealError {
    /// A required seal field was omitted during construction/validation.
    #[error("Missing required field: {0}")]
    MissingField(&'static str),
    /// The embedded attestation payload failed structural or trust validation.
    #[error("Invalid attestation")]
    InvalidAttestation,
    /// The Digital Seal signature did not verify against the declared signer.
...
mod tests {
    ...
    #[test]
    fn test_bincode_roundtrip_digital_seal() {
        let keypair = HybridKeypair::generate();
        let report = EnclaveReport::new([1u8; 32], [2u8; 32], [3u8; 64], HardwareType::AwsNitro);
        ...
        let encoded = bincode::serialize(&seal).expect("serialize seal");
        let decoded: DigitalSeal = bincode::deserialize(&encoded).expect("deserialize seal");

        assert_eq!(decoded.id.0, seal.id.0);
        assert_eq!(decoded.attestation.hardware, HardwareType::AwsNitro);
        assert_eq!(decoded.attestation.report.report_data, [3u8; 64]);
