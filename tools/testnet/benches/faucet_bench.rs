...
fn bench_faucet_drip_rate(c: &mut Criterion) {
    c.bench_function("faucet_drip_single", |b| {
        b.iter(|| {
            ...
            let address = "aeth1testaddr000000000000000000000000000";
            let _valid = address.starts_with("aeth1") && address.len() >= 39;
            let _amount: u64 = 1_000_000; // 1 AETH in uaeth
        });
    });
...
fn bench_faucet_batch(c: &mut Criterion) {
    c.bench_function("faucet_batch_100_drips", |b| {
        let addresses: Vec<String> = (0..100)
            .map(|i| format!("aeth1testaddr{:040}", i))
            .collect();

        b.iter(|| {
            let mut total: u64 = 0;
            for addr in &addresses {
                if addr.starts_with("aeth1") {
                    total += 1_000_000;
                }
            }
            total
