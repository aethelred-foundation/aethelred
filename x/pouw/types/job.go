...
// JobFeeMetadataKey is the metadata key under which a job's fee is recorded (as a
// Coin string, e.g. "100uaethel"). The reserved "scheduler." prefix prevents
// user-supplied metadata from overwriting it. The fee is stored here rather than
// in the proto Fee field because ComputeJob is a protobuf-v2 message and its Coin
// field maps to the gogoproto sdk.Coin, which the v2 marshaler cannot serialize.
const JobFeeMetadataKey = "scheduler.fee"
...
func NewComputeJobWithBlockTime(
	...
	job := &ComputeJob{
		...
		UpdatedAt:   now,
		ExpiresAt:   expiresAt,
		// The fee is recorded in Metadata, NOT in the proto Fee field. ComputeJob
		// is generated by protoc-gen-go (protobuf v2), but cosmos.base.v1beta1.Coin
		// maps to the gogoproto sdk.Coin whose Amount is math.Int — a custom type
		// the v2 reflection marshaler cannot serialize ("invalid Go type math.Int"),
		// which panics when the job is written to the store. The Fee field is not
		// read anywhere, so the amount is preserved as a string under a reserved
		// (scheduler.*) metadata key that user-supplied metadata cannot overwrite.
		Priority:            0,
		BlockHeight:         blockHeight,
		VerificationResults: make([]*VerificationResult, 0),
		Metadata:            map[string]string{JobFeeMetadataKey: fee.String()},
	}
