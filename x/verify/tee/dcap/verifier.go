...
func (v *DCAPVerifier) serializeReportBody(body *ReportBody) []byte {
	...
	_ = binary.Write(buf, binary.LittleEndian, body.MiscSelect)
	buf.Write(body.Reserved1[:])
	buf.Write(body.ISVExtProdID[:])
	_ = binary.Write(buf, binary.LittleEndian, body.Attributes.Flags)
	_ = binary.Write(buf, binary.LittleEndian, body.Attributes.Xfrm)
	buf.Write(body.MRENCLAVE[:])
	buf.Write(body.Reserved2[:])
	buf.Write(body.MRSIGNER[:])
	buf.Write(body.Reserved3[:])
	buf.Write(body.ConfigID[:])
	_ = binary.Write(buf, binary.LittleEndian, body.ISVProdID)
	_ = binary.Write(buf, binary.LittleEndian, body.ISVSVN)
	_ = binary.Write(buf, binary.LittleEndian, body.ConfigSVN)
